...
class EMADynamicAnnualV65Test(QCAlgorithm):
    """
    V65: Test inline loop with EXACT V30 momentum conditions

    V64 uses `if condition: continue` style
    V30 uses `if condition:` with nested append

    This tests if that difference matters.
    """
    ...
    def fine_filter(self, fine):
        """V30's EXACT logic but with inline sector filtering"""
        current_key = self.get_half_year_key()
        refresh_months = [1, 7]
        ...
        # Inline sector filtering (like V58) instead of list comprehension
        excluded_sectors = [103, 104, 207]

        stock_metrics = []
        for x in fine:
            try:
                # Inline sector filter
                if x.asset_classification.morningstar_sector_code in excluded_sectors:
                    continue

                symbol = x.symbol

                # V30's EXACT nested structure (not continue style)
