...
class EMADynamicAnnualV88Improved(QCAlgorithm):
    """
    V88: FINAL IMPROVED REGIME STRATEGY
    
    QUALITY_BULL (2015-2019):
    - Quality filters: Large cap >$10B, profitable EPS, ROE>10%
    - 6m only momentum (no 3m requirement)
    - NO volatility filter
    - QUARTERLY refresh
    - Result: 21.6% CAGR, 0.94 Sharpe
    
    SPECULATIVE_BULL (2020+):
    - NO quality filters
    - High volatility (top 10%)
    - Semi-annual refresh  
    - Dual momentum (6m AND 3m > 0)
    - Result: 28.6% CAGR, 0.90 Sharpe
    """
    ...
    def initialize(self):
        ...
        # Initialize to correct regime for start date
    ...
    def get_refresh_key(self):
        """Different refresh frequency per regime - NO regime in key!"""
        year = self.time.year
        
        if self.current_regime == "QUALITY_BULL":
            q = (self.time.month - 1) // 3 + 1
            return f"{year}Q{q}"
        else:
            half = 1 if self.time.month <= 6 else 2
            return f"{year}H{half}"

    def get_refresh_months(self):
        if self.current_regime == "QUALITY_BULL":
            return [1, 4, 7, 10]  # Quarterly
        else:
            return [1, 7]  # Semi-annual
    ...
    def coarse_filter(self, coarse):
        ...
        if new_regime != self.current_regime:
            ...
            self.last_refresh_key = None  # Force universe refresh on regime change

        current_key = self.get_refresh_key()
        refresh_months = self.get_refresh_months()
    ...
    def fine_filter(self, fine):
        ...
        self.debug(f"Fine filter with regime: {regime} at {self.time}")
        ...
        for x in fine:
            try:
                ...
                # QUALITY_BULL: Apply quality filters
                ...
                # SPECULATIVE_BULL: Also require 3m momentum
                if regime == "SPECULATIVE_BULL":
        ...
        if regime == "QUALITY_BULL":
            # NO volatility filter for quality - just sort by momentum
            selected = stock_metrics

        elif regime == "SPECULATIVE_BULL":
            # HIGH volatility filter (top 10%)
            if len(vols) >= 10:
