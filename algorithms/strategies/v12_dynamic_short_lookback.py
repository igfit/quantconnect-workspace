"""
v12 Dynamic Universe - SHORTER LOOKBACK (63 days)

Hypothesis: 126-day momentum might be too slow for 2015-2019 market.
Try 63-day (3-month) lookback which may catch momentum shifts faster.
"""

from AlgorithmImports import *


class V12DynamicShortLookback(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # SHORTER LOOKBACK
        self.lookback_days = 63  # 3 months instead of 6
        self.accel_period = 10   # Shorter accel too
        self.top_n = 8
        ...
        # Regime filters - keep but use simpler version
    ...
    def rebalance(self):
        ...
        # Only go to cash if below 50 SMA
        if not above_50:
            self.liquidate()
            return
