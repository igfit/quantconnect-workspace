"""
v12 Dynamic Universe - ALL SECTORS with momentum filter

Key insight: In 2015-2019, different sectors led (not just tech).
This version:
1. Opens to ALL sectors (not just growth)
2. Selects top 50 by dollar volume from entire market
3. Lower market cap floor ($500M)
4. The TRADING SIGNALS will filter for momentum

This tests if sector restriction was hurting OOS.
"""

from AlgorithmImports import *


class V12DynamicAllSectors(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # Universe - ALL SECTORS, wider net
        self.universe_size = 100  # Larger pool
        self.min_market_cap = 500e6
        self.max_market_cap = 300e9
        self.min_price = 5
        self.min_avg_dollar_volume = 15e6
    ...
    def fine_filter(self, fine):
        ...
        # Market cap filter only - NO SECTOR FILTER
        filtered = [x for x in fine
                   if x.market_cap > self.min_market_cap
                   and x.market_cap < self.max_market_cap]

        # Exclude utilities and real estate (typically low momentum)
        excluded_sectors = [
        ...
        self.log(f"Universe (all sectors): {len(selected)} stocks at {self.time.date()}")
