...
class EMADynamicAnnualV51(QCAlgorithm):
    """
    V51: SIMPLER REGIME FILTER

    V50 was too complex. Simplify:
    - BULL: SPY > 200 SMA → Run V30 at full power
    - BEAR: SPY < 200 SMA → 100% Cash

    No NEUTRAL state, no reduced exposure. Binary: all-in or all-out.

    The goal is to avoid major bear markets (2022, 2018 Q4, 2015-2016 chop)
    while capturing the full bull market gains.
    """
    ...
    def initialize(self):
        ...
        # V30 parameters (used in BULL mode)
        ...
        # SPY for regime detection - just 200 SMA
    ...
    def on_data(self, data):
        ...
        # Simple regime check: SPY > 200 SMA?
        ...
        if was_bull and not self.is_bull_regime:
            self.debug(f"REGIME -> BEAR: SPY={spy_price:.2f} < SMA200={sma200:.2f}")
