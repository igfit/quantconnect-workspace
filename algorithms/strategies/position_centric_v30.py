...
class PositionCentricV30(QCAlgorithm):
    """
    V30: Ultra Concentrated Position-Centric (Target 30% CAGR)

    Key changes from V28:
    - Only 2 positions (maximum concentration)
    - Higher momentum threshold (40%+)
    - Tighter entry criteria
    - Keep the successful breakout entry
    """
    ...
    def initialize(self):
        ...
        self.entry_mom_6m = 0.40        # 40% 6-month momentum
        self.entry_mom_3m = 0.15        # 15% 3-month momentum
        self.entry_adx = 25             # Stronger trend

        # Exit
        self.trailing_stop_pct = 0.20   # 20% trailing
        self.hard_stop_pct = 0.15
        self.days_below_sma = 2
    ...
    def check_entry_signal(self, symbol):
        ...
        # Very strong 6-month momentum (40%+)
    ...
    def daily_signal_check(self):
        ...
        if len(self.positions) < self.max_positions:
            ...
            for symbol, mom in candidates[:slots_available]:
                price = self.securities[symbol].price
                weight = 0.50  # 50% per position (2 positions)
