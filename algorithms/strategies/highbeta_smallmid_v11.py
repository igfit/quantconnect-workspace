"""
High-Beta Small/Mid Cap v11 - HYBRID (combining v7's returns with v9's DD control)

Target: 30% CAGR, 25-35% DD

Approach:
- v7's momentum-weighted allocation (not equal weight)
- v9's fast exit trigger (10 SMA)
- Mild leverage (1.2x) only in strong regime
- Biweekly rebalance (middle ground)
- Top 7 positions
"""
...
class HighBetaSmallMidV11(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.top_n = 7
        self.max_leverage = 1.2  # Mild leverage
        ...
        self.qqq_sma_10 = self.sma(self.qqq, 10, Resolution.DAILY)  # Fast exit
    ...
    def maybe_rebalance(self):
        ...
        if self.rebalance_week % 2 == 0:  # Every 2 weeks
            self.rebalance()
    ...
    def rebalance(self):
        ...
        if above_10 and above_20 and above_50 and qqq_mom_positive:
            leverage = self.max_leverage  # Strong regime
        elif above_10 and above_20:
            leverage = 1.0  # Normal regime
        else:
            leverage = 0.8  # Weak regime (reduce exposure)
