...
class EMADynamicAnnualV23(QCAlgorithm):
    """
    V23: PURE AGGRESSIVE MOMENTUM - NO DD CONTROL

    No DD control, no profit taking - pure momentum capture:
    - 5 positions at 20% (1x leverage)
    - Top 20 highest momentum stocks with top quartile volatility
    - 15/60 EMAs (faster than 20/100, slower than 10/50)
    - Only enter if stock's short-term momentum (1-month) > 10%

    Target: 28%+ CAGR (accepting higher DD for higher returns)
    """
    ...
    def fine_filter(self, fine):
            ...
            try:
                ...
                if len(history) >= self.momentum_lookback:
                    ...
                    if start_price > 0:
                        ...
                        # Only include if positive momentum AND short-term momentum > 10%
                        if momentum > 0 and short_mom > 0.10:
                            stock_metrics.append((symbol, momentum, vol, short_mom))
            except:
                pass
        ...
        self.short_momentum = {x[0]: x[3] for x in high_vol[:self.universe_size]}
        self.universe_symbols = top_stocks
        self.last_refresh_year = current_year

        self.debug(f"V23 Universe: {len(top_stocks)} high-momentum stocks")
    ...
    def on_data(self, data):
        ...
        for symbol in list(self.current_universe | self.held_symbols):
            ...
            # Entry on bullish cross (only if in universe)
