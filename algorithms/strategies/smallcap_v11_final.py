...
class SmallCapV11Final(QCAlgorithm):
    """
    Small-Cap V11: V10 + Mild Trend Filter

    Adds to V10:
    - Mild trend filter: Price > 20 SMA (not strict 50 SMA alignment)
    - Quicker exit when price breaks below 20 SMA
    - Slightly tighter relative strength (top 45%)
    """
    ...
    def initialize(self):
        ...
        self.max_single_position = 0.15  # Slightly smaller
        self.max_total_exposure = 0.70   # Slightly lower
        self.prev_prices = {}

        self.stop_atr_mult = 2.5
        self.trailing_atr_mult = 3.0
        self.trailing_activation = 0.10
        self.vix_entry_threshold = 27    # Tighter
        self.vix_exit_threshold = 34

        self.rs_percentile = 0.45  # Tighter - top 45%
    ...
    def is_above_20sma(self, ticker):
        """Mild trend filter - just check price > 20 SMA"""
        if ticker not in self.sma20 or not self.sma20[ticker].is_ready:
            return True  # Default to True if no data
        symbol = self.symbols[ticker]
        price = self.securities[symbol].price
        return price > self.sma20[ticker].current.value
    ...
    def check_exits(self):
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            # Exit if dropped below 20 SMA
            if not self.is_above_20sma(ticker) and pnl < 0.05:
                should_exit = True
    ...
    def rebalance(self):
        ...
        for ticker in self.symbols.keys():
            ...
            # Must be in top performers
            if ticker not in leaders:
                continue

            # Mild trend filter: price above 20 SMA
            if not self.is_above_20sma(ticker):
                continue
