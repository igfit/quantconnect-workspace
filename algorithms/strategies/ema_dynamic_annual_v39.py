...
class EMADynamicAnnualV39(QCAlgorithm):
    """
    V39: V38 + SOFT DD CONTROL

    V38 base (6 pos @ 17.5%, 25% trailing) + soft DD control:
    - Only reduce at 30% DD (not 20%)
    - Exit at 40% DD (not 30%)
    - Re-enter at 20% DD

    Target: 30%+ CAGR with <38% DD
    """
    ...
    def initialize(self):
        ...
        # Soft DD control - higher thresholds
        self.high_water_mark = 100000
        self.dd_reduce_threshold = 0.30  # Was 0.20
        self.dd_exit_threshold = 0.40    # Was 0.30
        self.dd_reentry_threshold = 0.20 # Was 0.12
    ...
    def on_data(self, data):
        ...
        if current_dd >= self.dd_reduce_threshold:
            if not self.in_drawdown_mode and len(self.held_symbols) > 3:
                ...
                for symbol, _ in held_with_scores[3:]:
