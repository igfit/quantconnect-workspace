...
class EMADynamicAnnualV6(QCAlgorithm):
    """
    EMA Crossover with Dynamic Universe - V6 WITH ATR STOP

    Same as V5 but with ATR trailing stop to reduce drawdown:
    - Exit if price drops 2x ATR from highest price since entry
    - Should cut losses faster while letting winners run

    Target: 22%+ CAGR with <40% DD
    """
    ...
    def initialize(self):
        ...
        self.highest_price = {}  # Track highest price since entry
    ...
    def on_data(self, data):
        ...
        for symbol in list(symbols_to_check):
            ...
            if symbol in self.held_symbols:
                ...
                # Check ATR stop
                stop_price = self.highest_price[symbol] - (self.atr_multiplier * atr)
                if current_price < stop_price:
                    self.liquidate(symbol, "ATR Stop")
        ...
        # Enter highest momentum candidates first
