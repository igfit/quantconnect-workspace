...
class SmallCapDynamicV2(QCAlgorithm):
    """
    V2: Add revenue growth filter + higher volatility stocks

    Hypothesis: High-growth companies with momentum = better returns
    """
    ...
    def initialize(self):
        ...
        # Universe: focus on growth + volatility
        self.min_market_cap = 300_000_000      # $300M
        self.max_market_cap = 8_000_000_000    # $8B
        self.min_price = 8
        self.min_dollar_volume = 8_000_000     # $8M/day
        self.min_revenue_growth = 0.15         # 15% YoY revenue growth
        self.universe_size = 20                # Smaller, more concentrated
        ...
        self.max_positions = 4                 # Even more concentrated
        ...
        self.rs_percentile = 0.40              # Top 40% only
    ...
    def fine_filter(self, fine):
        ...
        for x in fine:
            ...
            # Revenue growth filter - key differentiator
            try:
                rev_growth = x.operation_ratios.revenue_growth.one_year
                if rev_growth is None or rev_growth < self.min_revenue_growth:
                    continue
            except:
            ...
            try:
                ...
                growth_sectors = [311, 102, 308, 206]  # Tech, Consumer, Comm, Healthcare
                if sector not in growth_sectors:
                    continue
            except:
                pass  # Allow if sector data unavailable

            filtered.append((x, rev_growth))

        # Sort by revenue growth (best growth first)
        sorted_by_growth = sorted(filtered, key=lambda t: t[1], reverse=True)
        selected = [x[0].symbol for x in sorted_by_growth[:self.universe_size]]
        return selected
    ...
    def rebalance(self):
        ...
        for symbol in self.universe_symbols:
            ...
            if short_mom <= 0.012:  # Lower threshold for growth stocks
                continue
