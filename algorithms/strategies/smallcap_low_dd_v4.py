...
class SmallCapLowDDV4(QCAlgorithm):
    """
    Round 18 DD Reduction V4: Aggressive Risk Control

    Combining best features:
    1. Volatility-scaled positions (from V2) - but more aggressive
    2. VIX < 22 filter (aggressive)
    3. SPY must be above 50 SMA (shorter-term trend)
    4. Max 15% per position (was 25%)
    5. Max 60% total portfolio exposure
    6. ATR-based stops with 1.5x multiplier (tighter)
    7. Exit immediately on VIX spike above 28
    """
    ...
    def initialize(self):
        ...
        self.max_total_exposure = 0.60    # Max 60% total exposure
        self.prev_prices = {}

        # Aggressive parameters
        self.stop_atr_mult = 1.5          # Tighter ATR stop
        self.trailing_atr_mult = 2.0
        self.trailing_activation = 0.08
        self.vix_entry_threshold = 22     # No entries above VIX 22
        self.vix_exit_threshold = 28      # Exit all if VIX > 28
    ...
    def calculate_position_size(self, ticker):
        ...
        # Target risk per position: 1.5% of portfolio (more conservative)
        target_risk = 0.015
        ...
        # VIX scaling - more aggressive reduction
        vix = self.get_vix()
        if vix > 18:
    ...
    def check_exits(self):
        ...
        if vix > self.vix_exit_threshold:
            self.debug(f"{self.time.date()}: VIX EMERGENCY EXIT ({vix:.1f})")
            self.liquidate()
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            # SPY below 50 SMA - reduce exposure
            if self.spy_sma_50.is_ready:
                spy_price = self.securities[self.spy].price
                if spy_price < self.spy_sma_50.current.value * 0.98:  # 2% below
                    should_exit = True
                    reason = f"SPY_WEAK({pnl:+.1%})"
    ...
    def rebalance(self):
        ...
        if not self.spy_sma_50.is_ready or spy_price < self.spy_sma_50.current.value:
            return  # No new entries but hold existing
        ...
        for s in scores[:slots]:
            ...
            # Don't exceed max total exposure
