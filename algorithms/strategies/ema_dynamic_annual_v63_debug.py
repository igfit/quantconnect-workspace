...
class EMADynamicAnnualV63Debug(QCAlgorithm):
    """
    V63: DEBUG - V30 exact code with regime infrastructure wrapper

    This should produce IDENTICAL results to V30 (28.58% CAGR)
    Testing if the regime infrastructure itself causes any difference.
    """
    ...
    def initialize(self):
        ...
        # Regime infrastructure (should have no effect for 2020-2025)
        self.regime_dates = [
    ...
    def get_half_year_key(self):
        # V30 style - no regime in key
        year = self.time.year
        half = 1 if self.time.month <= 6 else 2
        return f"{year}H{half}"

    def coarse_filter(self, coarse):
        # Update regime (but don't affect caching)
    ...
    def fine_filter(self, fine):
        ...
        # V30 exact volatility filter (no fallback)
