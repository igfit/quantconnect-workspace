...
class EMADynamicAnnualV31(QCAlgorithm):
    """
    V31: V27 + ADAPTIVE POSITION SIZING

    Use portfolio volatility to adjust position sizes:
    - Start with 25% per position (1.25x leverage with 5 positions)
    - Reduce when 20-day portfolio volatility > 30% annualized
    - Scale down to 15% per position in high vol periods

    Target: 30%+ CAGR with <45% DD
    """
    ...
    def initialize(self):
        ...
        self.base_weight = 0.25    # Normal: 125% exposure
        self.reduced_weight = 0.15 # High vol: 75% exposure
        self.vol_threshold = 0.30  # 30% annualized volatility
        ...
        # Portfolio tracking for volatility
    ...
    def get_current_weight(self):
        """Calculate position weight based on recent portfolio volatility"""
        if len(self.portfolio_returns) < 20:
            return self.base_weight

        recent_returns = self.portfolio_returns[-20:]
    ...
    def on_data(self, data):
        ...
        if self.last_portfolio_value > 0:
            ...
            if len(self.portfolio_returns) > 60:  # Keep last 60 days
        ...
        if entry_candidates and position_count < self.max_positions:
            ...
            for symbol, _ in entry_candidates:
                if position_count >= self.max_positions:
                    break
                self.set_holdings(symbol, current_weight)
