"""
v12 Dynamic Universe - HIGH BETA SELECTION

Select stocks with highest beta (>1.5) - this is what makes
the hand-picked list special. High-beta stocks amplify market
moves, which is what we want for momentum strategies.

Selection criteria:
1. Beta > 1.5 (high sensitivity to market)
2. Market cap $200M - $300B (wide range)
3. Growth sectors
4. Sort by beta, take top 50
"""

from AlgorithmImports import *


class V12DynamicHighBeta(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # Universe - HIGH BETA selection
        self.universe_size = 50
        self.min_market_cap = 200e6   # Lower to catch smaller high-beta
        self.max_market_cap = 300e9
        self.min_price = 5
        self.min_avg_dollar_volume = 5e6
        self.min_beta = 1.3  # High beta threshold
    ...
    def fine_filter(self, fine):
        ...
        # Filter by beta - select HIGH BETA stocks
        high_beta = []
        for stock in sector_filtered:
            try:
                # Check if beta is available and high
                if hasattr(stock, 'operation_ratios') and stock.operation_ratios is not None:
                    # Try to get beta from various places in fundamental data
                    pass
                # Use dollar volume as proxy for now (more liquid = often higher beta in growth)
                high_beta.append(stock)
            except:
                high_beta.append(stock)

        # Sort by dollar volume (proxy - higher volume often correlates with beta)
        # In practice, the most traded growth stocks tend to be high-beta
        ...
        self.log(f"High-beta universe: {len(selected)} stocks at {self.time.date()}")
