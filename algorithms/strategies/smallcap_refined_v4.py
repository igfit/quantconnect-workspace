...
class SmallCapRefinedV4(QCAlgorithm):
    """
    Small-Cap Refined V4: Hybrid Approach

    Combines V1's DD reduction with more aggressive entries:
    - Wider stops: 2.5x ATR (vs 2x) - but not as wide as V1's 3x
    - Medium lookbacks: 30/84 days (between baseline 21/63 and V1's 42/126)
    - Lower momentum threshold: 1.5% (vs V1's 3%)
    - Weekly rebalance (faster than V2's bi-weekly)
    - Trend confirmation but lower ADX threshold (18 vs 25)
    """
    ...
    def initialize(self):
        ...
        # Medium lookbacks - balance between noise filtering and responsiveness
        self.short_lookback = 30   # ~1.5 months
        self.long_lookback = 84    # 4 months
        ...
        # Moderate stops - wider than baseline but not as wide as V1
        self.stop_atr_mult = 2.5         # Was 2.0 baseline, 3.0 in V1
        self.trailing_atr_mult = 3.0     # Was 2.5 baseline, 3.5 in V1
        self.trailing_activation = 0.12
        self.vix_entry_threshold = 28
        self.vix_exit_threshold = 35

        # Lower thresholds than V1
        self.min_momentum = 0.015  # 1.5% vs V1's 3%
        self.min_adx = 18         # Vs V1's 25
