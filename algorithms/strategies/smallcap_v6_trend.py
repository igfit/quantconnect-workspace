...
class SmallCapV6TrendFilter(QCAlgorithm):
    """
    Small-Cap V6: Stock-Level Trend Filter

    Key idea: Only trade stocks that are IN an uptrend (not downranging)
    - Price must be above 50-day SMA
    - Price must be above 20-day SMA (shorter trend)
    - 20 SMA must be above 50 SMA (trend alignment)

    This avoids buying falling knives and stocks in downtrends.
    """
    ...
    def initialize(self):
        ...
        for ticker in self.tickers:
            try:
                ...
                self.sma20[ticker] = self.sma(sym, 20, Resolution.DAILY)
                self.sma50[ticker] = self.sma(sym, 50, Resolution.DAILY)
            except:
                pass
    ...
    def is_in_uptrend(self, ticker):
        """Check if stock is in an uptrend"""
        if ticker not in self.sma20 or ticker not in self.sma50:
            return False
        if not self.sma20[ticker].is_ready or not self.sma50[ticker].is_ready:
            return False

        symbol = self.symbols[ticker]
        price = self.securities[symbol].price
        sma20_val = self.sma20[ticker].current.value
        sma50_val = self.sma50[ticker].current.value

        # All three conditions must be true
        price_above_20 = price > sma20_val
        price_above_50 = price > sma50_val
        sma_aligned = sma20_val > sma50_val

        return price_above_20 and price_above_50 and sma_aligned
    ...
    def check_exits(self):
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            # Trend reversal - exit if price drops below 20 SMA
            if ticker in self.sma20 and self.sma20[ticker].is_ready:
                if price < self.sma20[ticker].current.value * 0.98:  # 2% buffer
                    should_exit = True
    ...
    def rebalance(self):
        ...
        for ticker in self.symbols.keys():
            ...
            # KEY FILTER: Must be in uptrend
            if not self.is_in_uptrend(ticker):
                continue
