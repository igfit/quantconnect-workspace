...
class EMADynamicAnnualV30(QCAlgorithm):
    """
    V30: V27 + TRAILING STOP ONLY (no profit taking)

    Different approach: Instead of profit taking, use trailing stop on ALL positions
    from the start. This lets winners run while protecting against large reversals.

    - 25% trailing stop from highest price
    - No partial profit taking

    Target: 30%+ CAGR with <50% DD
    """
    ...
    def initialize(self):
        ...
        # Trailing stop only
        self.trailing_stop_pct = 0.25  # Exit if drops 25% from high
