...
class SmallCapV7RelativeStrength(QCAlgorithm):
    """
    Small-Cap V7: Relative Strength Filter

    Key idea: Only trade stocks that are OUTPERFORMING the universe
    - Calculate 21-day return for each stock
    - Only trade stocks in top 40% of universe
    - This filters out laggards and focuses on leaders

    Combined with faster entry/exit (shorter lookbacks for signals).
    """
    ...
    def initialize(self):
        ...
        # Faster signals
        self.short_lookback = 21  # Back to faster
        self.long_lookback = 63
        self.beta_lookback = 126
        self.rs_lookback = 21  # Relative strength lookback
        ...
        self.trailing_activation = 0.10  # Faster trailing activation
        self.vix_entry_threshold = 28
        self.vix_exit_threshold = 35

        # Relative strength threshold (top 40%)
        self.rs_percentile = 0.40
    ...
    def calculate_universe_returns(self):
        """Calculate recent return for each stock"""
        returns = {}
    ...
    def check_exits(self):
        ...
        # Get current leaders for exit check
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            # Exit if stock drops out of leaders (relative strength exit)
            if ticker not in leaders and pnl < 0.05:  # Only if not profitable
                should_exit = True
    ...
    def rebalance(self):
        ...
        for ticker in self.symbols.keys():
            ...
            # KEY FILTER: Must be in top performers
