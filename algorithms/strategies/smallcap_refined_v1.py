...
class SmallCapRefinedV1(QCAlgorithm):
    """
    Small-Cap Refined V1: Wider Stops + Longer Lookbacks

    Changes from baseline:
    - Longer lookbacks: 42/126 days (vs 21/63) to filter noise
    - Wider stops: 3x ATR (vs 2x) to avoid whipsaws
    - Higher momentum threshold: 3% (vs 1%)
    ...
    def initialize(self):
        ...
        # Longer lookbacks to filter noise
        self.short_lookback = 42   # 2 months (was 21)
        self.long_lookback = 126   # 6 months (was 63)
        ...
        self.max_single_position = 0.15  # Smaller positions for volatile stocks
        self.max_total_exposure = 0.70   # Lower exposure
        self.prev_prices = {}

        # Wider stops for volatile stocks
        self.stop_atr_mult = 3.0         # Was 2.0
        self.trailing_atr_mult = 3.5     # Was 2.5
        self.trailing_activation = 0.15  # Higher threshold to activate trailing
        self.vix_entry_threshold = 28    # More lenient for volatile stocks
        self.vix_exit_threshold = 35

        # Higher momentum threshold
        self.min_momentum = 0.03  # 3% vs 1%
        self.min_adx = 25         # Stricter trend filter
    ...
    def get_atr_pct(self, ticker):
        if ticker not in self.atr_ind or not self.atr_ind[ticker].is_ready:
            return 0.05  # Higher default for small-caps
        symbol = self.symbols[ticker]
    ...
    def calculate_position_size(self, ticker):
        ...
        target_risk = 0.015  # Lower risk per position
    ...
    def check_exits(self):
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            short_result = self.calculate_residual_momentum(ticker, self.short_lookback)
            if short_result is not None and short_result[0] < -0.03:
                should_exit = True
    ...
    def rebalance(self):
        ...
        for ticker in self.symbols.keys():
            ...
            if short_mom <= self.min_momentum:
                continue
            ...
            if ticker in self.adx_ind and self.adx_ind[ticker].is_ready:
                ...
                pos_di = self.adx_ind[ticker].positive_directional_index.current.value
                neg_di = self.adx_ind[ticker].negative_directional_index.current.value
                if adx < self.min_adx or pos_di <= neg_di:
                    continue
                score = short_mom * (adx / 100) * beta
            else:
                continue  # Require ADX for small-caps
