...
class SmallCapV10Combined(QCAlgorithm):
    """
    Small-Cap V10: Best of V4 + V7

    Combines:
    - V4's medium lookbacks (30/84) for better signal
    - V7's relative strength filter (top 50% of universe)
    - V4's lower thresholds for more entries
    - Faster exits when stock drops out of top performers
    """
    ...
    def initialize(self):
        ...
        # Relative strength: top 50% (less restrictive than V7's 40%)
    ...
    def get_rs_leaders(self):
        ...
        if len(returns) < 5:
            return set(self.symbols.keys())  # Default to all if not enough data
