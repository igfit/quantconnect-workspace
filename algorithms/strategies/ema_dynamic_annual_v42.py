...
class EMADynamicAnnualV42(QCAlgorithm):
    """
    V42: DIVERSIFIED HIGH VOL

    Keep top decile volatility (essential for returns) but increase diversification:
    - 8 positions @ 14% = 112% exposure
    - 90th percentile volatility (like V34)
    - 28% trailing stop (between 25% and 30%)

    Target: 30%+ CAGR with <40% DD
    """
    ...
    def initialize(self):
        ...
        self.universe_size = 40  # Larger universe for more picks
        self.min_price = 10.0
        self.min_dollar_volume = 10_000_000
        self.momentum_lookback = 126
        self.short_momentum_lookback = 63

        # More diversification with slight leverage
        self.max_positions = 8
        self.weight_per_stock = 0.14

        self.ema_fast = 20
        self.ema_slow = 100
        self.trailing_stop_pct = 0.28  # Middle ground

        self.vol_percentile = 90  # Keep high vol for returns
