...
class EMADynamicAnnualV21(QCAlgorithm):
    """
    V21: ULTRA CONCENTRATED + MOMENTUM ACCELERATION

    Changes:
    - 4 positions at 25% (max concentration at 1x)
    - Only top 15 stocks (highest conviction)
    - Momentum acceleration filter: current momentum > last month's
    - 12/60 EMAs (balanced speed)
    - Only trade when stock's 20-day momentum > 0

    Max 1x leverage: 4 positions at 25%
    Target: 28%+ CAGR with <35% DD
    """
    ...
    def initialize(self):
        ...
        self.universe_size = 15  # Ultra concentrated universe
        self.min_price = 15.0  # Higher price floor
        self.min_dollar_volume = 20_000_000  # More liquid
        ...
        # Profit taking
        self.profit_take_threshold = 0.35
    ...
    def fine_filter(self, fine):
        ...
        growth_sectors = [311, 309, 308, 310]  # Tech, Healthcare, Consumer Cyclical, Communication
        ...
        for symbol in [x.symbol for x in filtered]:
            try:
                history = self.history(symbol, self.momentum_lookback + 21, Resolution.DAILY)
                if len(history) >= self.momentum_lookback:
                    # 6-month momentum
                    start_price = history['close'].iloc[-self.momentum_lookback]
                    end_price = history['close'].iloc[-1]

                    # 1-month ago momentum (for acceleration check)
                    start_price_1m = history['close'].iloc[-(self.momentum_lookback - 21)]
                    end_price_1m = history['close'].iloc[-21]

                    if start_price > 0 and start_price_1m > 0:
                        momentum_now = (end_price - start_price) / start_price
                        momentum_1m_ago = (end_price_1m - start_price_1m) / start_price_1m

                        # Only include if momentum is accelerating
                        if momentum_now > 0 and momentum_now > momentum_1m_ago:
                            returns = history['close'].pct_change().dropna()
                            vol = returns.std() * np.sqrt(252)
                            stock_metrics.append((symbol, momentum_now, vol))
            except:
                pass
        ...
        self.debug(f"V21 Universe: {len(top_stocks)} accelerating momentum stocks")
    ...
    def on_data(self, data):
        ...
        for symbol in list(self.current_universe | self.held_symbols):
            ...
            ema_20 = ind['ema_20'].current.value if ind['ema_20'].is_ready else current_price
            ...
            # Additional filter: only enter if price > 20 EMA (short-term uptrend)
            short_term_bullish = current_price > ema_20
            ...
            elif bullish_cross and symbol in self.current_universe and short_term_bullish:
