...
class EMADynamicAnnualV67Test(QCAlgorithm):
    """
    V67: V66 + regime volatility branching (but no QUALITY_BULL filter)

    Tests if the regime-specific volatility filter causes the difference.
    """
    ...
    def fine_filter(self, fine):
        """V66 + regime volatility branching"""
        current_key = self.get_half_year_key()
        refresh_months = [1, 7]
        ...
        for x in fine:
            try:
                ...
                # NO QUALITY_BULL filters here - just continue style momentum
