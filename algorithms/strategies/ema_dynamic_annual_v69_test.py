...
class EMADynamicAnnualV69Test(QCAlgorithm):
    """
    V69: V68 but use regime check ONLY for SPECULATIVE_BULL branch

    Tests if the if/elif structure causes Python scoping issues.
    """
    ...
    def fine_filter(self, fine):
        """Regime check but ONLY SPECULATIVE_BULL logic (no QUALITY_BULL code at all)"""
        current_key = self.get_half_year_key()
        refresh_months = [1, 7]
        ...
        self.debug(f"Fine filter regime: {regime}")
        ...
        # ONLY SPECULATIVE_BULL volatility filter - no if/elif
        ...
        self.debug(f"Universe: {len(top_stocks)} stocks")
