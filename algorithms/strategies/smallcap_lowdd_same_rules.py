...
class SmallCapLowDDSameRules(QCAlgorithm):
    """
    Small/Mid-Cap with SAME DD reduction rules as LargeCapLowDD

    Testing if DD reduction techniques work across different universes.
    Uses IDENTICAL risk parameters as LargeCapLowDD, only universe differs.

    LargeCapLowDD achieved: Sharpe 1.046, CAGR 26.2%, MaxDD 19.6%
    """
    ...
    def initialize(self):
        ...
        # Small/Mid-Cap High-Beta Universe (SAME as R18)
        self.tickers = [
            # Fintech
        ...
        # === IDENTICAL PARAMETERS AS LargeCapLowDD ===
        self.max_positions = 5
        self.max_single_position = 0.18
        self.max_total_exposure = 0.80
        self.prev_prices = {}

        # Risk parameters - SAME as LargeCapLowDD
    ...
    def calculate_position_size(self, ticker):
        ...
        # Target risk per position: 2% of portfolio (SAME as LargeCapLowDD)
        target_risk = 0.02
        ...
        # VIX scaling (SAME as LargeCapLowDD)
        vix = self.get_vix()
        if vix > 20:
    ...
    def check_exits(self):
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            # ATR-based stop (SAME as LargeCapLowDD)
            stop_price = entry - (self.stop_atr_mult * atr)
            if price <= stop_price:
                should_exit = True
                reason = f"ATR_STOP({pnl:.1%})"

            # ATR-based trailing stop (SAME as LargeCapLowDD)
            ...
            # ADX trend reversal (SAME as LargeCapLowDD)
    ...
    def rebalance(self):
        ...
        for ticker in self.symbols.keys():
            ...
            # Same threshold as original small-cap
