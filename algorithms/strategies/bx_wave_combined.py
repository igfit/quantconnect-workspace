...
class BXWaveCombinedStrategy(QCAlgorithm):
    """
    Combined BX Trender + WaveTrend Strategy

    Strategy:
        - Go long when BOTH conditions are met:
          1. BX Trender is bullish (green) or just turned bullish
          2. WaveTrend is coming out of oversold zone (<-60)
        - Exit when BX Trender turns bearish OR WaveTrend hits overbought

    This combines trend direction (BX) with mean reversion timing (WaveTrend).
    ...
    def initialize(self):
        ...
        self.esa_window = RollingWindow[float](50)
    ...
    def on_data(self, data):
        ...
        if self.prev_short_xt is not None and self.prev_wt1 is not None and self.prev_wt2 is not None:
            ...
            bx_just_turned_bullish = self.prev_short_xt < 0 and short_xt >= 0
            bx_just_turned_bearish = self.prev_short_xt >= 0 and short_xt < 0

            # WaveTrend state
            wt_oversold = wt1 < self.oversold or self.prev_wt1 < self.oversold
            wt_overbought = wt1 > self.overbought
            wt_cross_up = self.prev_wt1 < self.prev_wt2 and wt1 > wt2

            # BUY: BX is bullish AND WaveTrend coming out of oversold
            buy_signal = (bx_bullish or bx_just_turned_bullish) and wt_oversold and wt_cross_up

            # SELL: BX turns bearish OR WaveTrend hits overbought
            sell_signal = bx_just_turned_bearish or (wt_overbought and self.prev_wt1 > self.prev_wt2 and wt1 < wt2)

            if buy_signal and not self.portfolio[self.symbol].invested:
                self.set_holdings(self.symbol, 1.0)
                self.debug(f"{self.time}: BUY - BX bullish ({short_xt:.1f}) + WT oversold cross ({wt1:.1f})")

            elif sell_signal and self.portfolio[self.symbol].invested:
                self.liquidate(self.symbol)
                reason = "BX bearish" if bx_just_turned_bearish else "WT overbought"
