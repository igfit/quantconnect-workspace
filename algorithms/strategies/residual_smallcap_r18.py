...
class ResidualSmallCapR18(QCAlgorithm):
    """
    Round 18 Strategy 1: Residual Momentum on Small/Mid-Cap High-Beta

    Testing if residual momentum signal transfers to smaller caps.
    Universe: Russell 2000 high-beta stocks (beta > 1.5)

    Same logic as R16 Residual HighBeta, different universe.
    """
    ...
    def initialize(self):
        ...
        # Small/Mid-Cap High-Beta Universe (Russell 2000 + growth names)
        # Focus on volatile sectors: fintech, EV, crypto-adjacent, gaming, cloud
        self.tickers = [
            # Fintech (high beta, volatile)
            ...
            # EV / Clean Energy (very high beta)
            "RIVN",    # Rivian
            "LCID",    # Lucid Motors
            "PLUG",    # Plug Power
    ...
    def calculate_beta(self, stock_returns, market_returns):
        if len(stock_returns) < 60 or len(market_returns) < 60:
            return 1.5  # Assume high beta for small caps
    ...
    def check_exits(self):
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            # Wider stop for small caps (more volatile)
            if pnl <= -0.10:
                should_exit = True
                reason = f"STOP({pnl:.1%})"

            # Trailing stop after 12% gain
            if pnl >= 0.12:
    ...
    def rebalance(self):
        ...
        vix = self.get_vix()
        if vix > 35:  # Higher VIX threshold for small caps
            return
        ...
        for ticker in self.symbols.keys():
            ...
            # Higher threshold for small caps (more noise)
