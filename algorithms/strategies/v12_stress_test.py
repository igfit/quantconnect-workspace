"""
v12 Strategy - STRESS TEST VERSION

Realistic assumptions for validation:
1. Higher slippage: 0.2% (double the original)
2. Higher min volume: $10M (ensures liquidity)
3. Explicit next-day open execution
4. Remove stocks that IPO'd after 2019 (survivorship bias check)
5. More conservative position sizing
"""

from AlgorithmImports import *


class V12StressTest(QCAlgorithm):
    ...
    def initialize(self):
        ...
        # MORE REALISTIC: Higher slippage (0.2% instead of 0.1%)
        self.set_security_initializer(lambda security: security.set_slippage_model(
            ConstantSlippageModel(0.002)  # 0.2% slippage
        ))
        ...
        self.min_dollar_volume = 10_000_000  # Higher threshold

        self.prev_short_mom = {}
        self.rebalance_week = 0
        self.pending_orders = {}

        # SURVIVORSHIP BIAS FIX: Only stocks that existed before 2020
        # Removed: CRWD (2019 IPO), UBER (2019 IPO), SNOW (2020), DDOG (2019)
        # Keeping only stocks with 5+ year history as of 2020
        self.universe_tickers = [
            # Tech leaders (pre-2019)
            "TSLA", "NVDA", "AMD",

            # Semiconductors (all pre-2019)
            "MU", "MRVL", "ON", "SWKS", "AMAT", "LRCX", "KLAC",

            # Software (pre-2019 only)
            "ZS", "OKTA", "TWLO", "MDB",  # Removed CRWD (June 2019 IPO)

            # Fintech (pre-2019)
            "SQ", "PYPL",

            # E-commerce/Consumer (pre-2019)
            "SHOP", "ETSY", "ROKU", "SNAP", "PINS", "TTD",

            # Clean Energy (pre-2019)
            "ENPH", "SEDG", "FSLR",

            # Travel (pre-2019 only)
            "EXPE",  # Removed UBER (May 2019 IPO)
        ...
        # Schedule signal generation on Monday close, execute Tuesday open
        self.schedule.on(
        ...
        # Execute orders at market open next day
        self.schedule.on(
    ...
    def generate_signals(self):
        """Generate signals at Monday close, to be executed Tuesday open"""
        ...
        # Fast exit
        if not above_10:
            self.target_weights = {}  # Signal to liquidate all
            return

        # Check for weak positions to exit
        weak_positions = set()
    ...
    def execute_orders(self):
        """Execute orders at Tuesday open (next-day execution)"""
        if self.is_warming_up:
            return

        # If target_weights is empty, liquidate all
