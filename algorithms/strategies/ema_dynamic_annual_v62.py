...
class EMADynamicAnnualV62(QCAlgorithm):
    """
    V62: OPTIMIZED HARDCODED REGIME STRATEGY

    Based on testing V59-V61, using best parameters for each regime:

    QUALITY_BULL (2015-2019 style):
    - Large cap (>$10B)
    - Must be profitable (EPS > 0)
    - ROE > 10%
    - MODERATE volatility (30-70 percentile) - tested as best
    - Tighter trailing stop (20%) - less risky environment
    - 4 positions @ 20% = 80% exposure

    SPECULATIVE_BULL (2020-2024 style):
    - No market cap minimum
    - No quality filters
    - Top 10% volatility
    - Looser trailing stop (30%) - let winners run
    - 5 positions @ 22% = 110% exposure (slight leverage)

    REGIME CHARACTERISTICS for external identification:

    SPECULATIVE_BULL (bubble) signals:
    - VIX < 15 for extended periods
    - IPO/SPAC frenzy
    - Retail trading surge
    - Growth >> Value
    - Narrow leadership (few stocks driving market)
    - Crypto mania correlation

    QUALITY_BULL (normal) signals:
    - VIX 15-25
    - Broad market participation
    - Quality/Value factors work
    ...
    def initialize(self):
        ...
        # Regime-specific defaults (will be overwritten)
    ...
    def update_regime_parameters(self):
        ...
        if regime == "QUALITY_BULL":
            ...
            self.weight_per_stock = 0.20  # 80% exposure
            self.trailing_stop_pct = 0.20  # Tighter stop
            self.min_market_cap = 10e9
            self.require_profitable = True
            self.require_roe = True
            self.vol_filter = "moderate"  # 30-70 percentile

        elif regime == "SPECULATIVE_BULL":
            # Aggressive, momentum-focused
            self.max_positions = 5
            self.weight_per_stock = 0.22  # 110% exposure (slight leverage)
            self.trailing_stop_pct = 0.30  # Let winners run
            self.min_market_cap = 0
            self.require_profitable = False
            self.require_roe = False
            self.vol_filter = "high"  # Top 10%
    ...
    def fine_filter(self, fine):
        ...
        # Ensure parameters are set for current regime
        self.update_regime_parameters()
        ...
        self.debug(f"Universe: {len(top_stocks)} stocks | Regime: {self.current_regime}")
