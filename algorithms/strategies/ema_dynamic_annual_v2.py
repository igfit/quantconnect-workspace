...
class EMADynamicAnnualV2(QCAlgorithm):
    """
    EMA Crossover with Dynamic Universe - ANNUAL REFRESH V2

    Improvements:
    - Wider EMAs (20/100) to reduce whipsaws
    - Select by momentum + minimum volatility (want volatile momentum stocks)
    - Top 40 stocks (more selective)

    Universe: Top 40 by momentum where volatility > median, refreshed annually
    Signal: EMA(20) crosses above EMA(100)
    Exit: EMA(20) crosses below EMA(100)
    """
    ...
    def initialize(self):
        ...
        # Position settings - more concentrated
        self.max_positions = 8
        self.weight_per_stock = 0.125

        # Wider EMAs to reduce whipsaws
        self.ema_fast = 20
        self.ema_slow = 100
    ...
    def fine_filter(self, fine):
        ...
        # Filter sectors (exclude financials, utilities, real estate)
        ...
        # Calculate both momentum AND volatility
        stock_metrics = []
        ...
        # Filter to stocks with above-median volatility (we want volatile momentum stocks)
        ...
        if volatile_momentum:
            top = volatile_momentum[0]
            self.debug(f"Top: {top[0].value} momentum={top[1]:.1%} vol={top[2]:.1%}")
