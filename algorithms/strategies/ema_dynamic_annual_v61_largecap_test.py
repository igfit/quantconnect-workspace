...
class EMADynamicAnnualV61LargeCapTest(QCAlgorithm):
    """
    V61: TEST LARGE CAP MOMENTUM WITHOUT QUALITY FILTERS (2015-2019)

    Testing if quality filters (profitable, ROE>10%) are hurting returns.

    Changes from V59/V60:
    - Keep large cap filter (>$10B)
    - REMOVE profitability requirement
    - REMOVE ROE requirement
    - Top 50% volatility

    Hypothesis: Maybe quality filters exclude growth stocks that aren't
    yet profitable but have strong momentum (like early AMZN, TSLA).
    """
    ...
    def fine_filter(self, fine):
        ...
        for x in fine:
            try:
                ...
                # ONLY large cap filter - NO quality filters
        ...
        # Top 50% volatility among large caps
        vols = [x[2] for x in stock_metrics]
        if len(vols) >= 10:
            vol_50th = np.percentile(vols, 50)
            selected = [x for x in stock_metrics if x[2] >= vol_50th]
        else:
