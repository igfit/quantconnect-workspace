...
class MomentumAccelSmallCap(QCAlgorithm):
    """
    Momentum Acceleration + DD Reduction - Small/Mid Cap Universe

    Tests if the strategy works without TSLA/NVDA/MSTR bias.
    Same logic as MomentumAccelLowDD but on small-cap speculative stocks.
    """
    ...
    def initialize(self):
        ...
        self.short_lookback = 21   # 1 month
        self.long_lookback = 63    # 3 months
        self.beta_lookback = 126

        # Small/Mid-Cap Universe (no TSLA, NVDA, MSTR)
        self.tickers = [
            # Fintech
