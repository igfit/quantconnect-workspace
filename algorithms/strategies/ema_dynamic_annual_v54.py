...
class EMADynamicAnnualV54(QCAlgorithm):
    """
    V54: MOMENTUM BREADTH REGIME STRATEGY

    Different approach: Use momentum breadth to detect market regime.
    - If SPY momentum strong AND breadth strong → EUPHORIA → aggressive
    - If SPY momentum weak OR breadth weak → CAUTIOUS → quality
    - If SPY < 200 SMA → BEAR → cash

    BREADTH INDICATOR:
    Track what % of our universe has positive 3m momentum.
    - > 70% = strong breadth (many winners) → aggressive
    - < 50% = weak breadth (few winners) → conservative

    This should help us be aggressive only when the environment supports it.
    """
    ...
    def initialize(self):
        ...
        self.highest_prices = {}
        self.stock_metrics_cache = []  # Cache for breadth calculation
    ...
    def update_regime(self):
        """Update regime based on SPY position and breadth"""
        ...
        # BEAR: SPY below 200 SMA
        if spy_price < sma200:
            self.current_regime = "BEAR"
            self.max_positions = 0
            self.weight_per_stock = 0
        # AGGRESSIVE: Strong breadth (>70%) AND SPY trending up
        elif self.breadth_ratio > 0.70 and spy_price > sma200 * 1.02:
            self.current_regime = "AGGRESSIVE"
            self.vol_percentile = 90  # Top 10% vol
            self.max_positions = 5
            self.weight_per_stock = 0.20  # 100% exposure
            self.trailing_stop_pct = 0.30
        # CAUTIOUS: Normal market or weak breadth
        else:
            self.current_regime = "CAUTIOUS"
            self.vol_percentile = 70  # Top 30% vol
            self.max_positions = 4
            self.weight_per_stock = 0.15  # 60% exposure
            self.trailing_stop_pct = 0.20

        if old_regime != self.current_regime:
            self.debug(f"REGIME: {old_regime} -> {self.current_regime} | Breadth={self.breadth_ratio:.1%}")

            # If entering BEAR, liquidate
    ...
    def fine_filter(self, fine):
        ...
        if total_count > 0:
            self.breadth_ratio = positive_momentum_count / total_count
        else:
            self.breadth_ratio = 0.5

        self.debug(f"Breadth: {positive_momentum_count}/{total_count} = {self.breadth_ratio:.1%}")

        if not stock_metrics:
            return list(self.held_symbols)

        # Apply volatility filter based on current regime parameters
        vols = [x[2] for x in stock_metrics]
        if len(vols) >= 10:
            vol_threshold = np.percentile(vols, self.vol_percentile)
            high_vol = [x for x in stock_metrics if x[2] >= vol_threshold]
        else:
