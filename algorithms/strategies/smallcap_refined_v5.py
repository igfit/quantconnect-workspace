...
class SmallCapRefinedV5(QCAlgorithm):
    """
    Small-Cap Refined V5: Best of V1 + V4

    Combines V4's better signal (medium lookbacks, lower thresholds)
    with V1's stricter risk controls (wider stops, lower exposure).

    Goal: Sharpe > 0.7, CAGR > 20%, MaxDD < 30%
    """
    ...
    def initialize(self):
        ...
        # V1's conservative position limits
        self.max_positions = 4         # Reduced from 5
        self.max_single_position = 0.14  # Reduced from 0.16
        self.max_total_exposure = 0.60   # Reduced from 0.75
        self.prev_prices = {}

        # V1's wide stops
        self.stop_atr_mult = 3.0
        self.trailing_atr_mult = 3.5
        self.trailing_activation = 0.15
        self.vix_entry_threshold = 26    # Tighter
        self.vix_exit_threshold = 32     # Earlier exit

        # V4's entry thresholds
        self.min_momentum = 0.02
        self.min_adx = 20

        # Drawdown-based position scaling (from CombinedBestLowDD)
        self.peak_equity = 100000
        self.dd_scale_start = 0.08
        self.dd_no_new_positions = 0.18
    ...
    def calculate_position_size(self, ticker):
        ...
        vix = self.get_vix()
        if vix > 18:
            vix_scale = max(0.4, 1.0 - (vix - 18) / 18)
    ...
    def rebalance(self):
        ...
        for s in scores[:slots]:
            ...
            if total_exposure + weight > self.max_total_exposure:
                weight = max(0.04, self.max_total_exposure - total_exposure)
            if weight < 0.04:
                continue
