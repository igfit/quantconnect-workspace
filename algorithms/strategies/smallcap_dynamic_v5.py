...
class SmallCapDynamicV5(QCAlgorithm):
    """
    V5: Remove regime filter + full invested + wider market cap

    Hypothesis: SPY > 200 SMA filter misses too many opportunities
    2020-2024 was mostly a bull market, should be fully invested
    """
    ...
    def initialize(self):
        ...
        self.rs_lookback = 10                   # Very short RS lookback

        # Wider market cap to catch all winners
        self.min_market_cap = 500_000_000       # $500M
        self.max_market_cap = 200_000_000_000   # $200B
        self.min_price = 10
        self.min_dollar_volume = 30_000_000     # $30M/day
        ...
        self.max_total_exposure = 1.0           # 100% invested

        self.stop_atr_mult = 2.5
        self.trailing_atr_mult = 3.0
        self.trailing_activation = 0.10
        self.vix_entry_threshold = 35           # Higher tolerance
        self.vix_exit_threshold = 45
        self.rs_percentile = 0.30

        # Rebalance 3x per week
        self.schedule.on(
        ...
        self.set_warm_up(100, Resolution.DAILY)  # Shorter warmup
    ...
    def calculate_position_size(self, ticker):
        ...
        vix = self.get_vix()
        if vix > 25:
            vix_scale = max(0.5, 1.0 - (vix - 25) / 30)
    ...
    def rebalance(self):
        ...
        # NO REGIME FILTER - always trade in bull market
        vix = self.get_vix()
        ...
        for symbol in self.universe_symbols:
            ...
            if short_mom <= 0.005:  # Very low threshold
                continue
