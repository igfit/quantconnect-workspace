...
class SmallCapV9NearHigh(QCAlgorithm):
    """
    Small-Cap V9: Near 52-Week High Filter

    Key idea: Only trade stocks that are NEAR their highs (trending, not falling)
    - Price must be within 15% of 52-week high
    - Avoids buying stocks that have crashed from their highs
    - Focuses on stocks with positive price structure

    Also includes pullback entry logic - buy the dip within an uptrend.
    """
    ...
    def initialize(self):
        ...
        self.symbols = {}
        self.return_history = {}
        self.price_history = {}  # Track price history for 52-week high
        self.adx_ind = {}
        self.atr_ind = {}
        self.entry_prices = {}
        self.entry_atr = {}
        ...
        self.high_lookback = 252  # ~1 year of trading days
        self.near_high_threshold = 0.15  # Within 15% of 52-week high
        ...
        self.set_warm_up(260, Resolution.DAILY)  # Extra warmup for 52-week high

    def is_near_52_week_high(self, ticker):
        """Check if price is within threshold of 52-week high"""
        if ticker not in self.price_history:
            return False
        prices = self.price_history[ticker]
        if len(prices) < self.high_lookback:
            return False

        high_52w = max(prices[-self.high_lookback:])
        current_price = prices[-1]

        distance_from_high = (high_52w - current_price) / high_52w
        return distance_from_high <= self.near_high_threshold

    def is_pullback_in_uptrend(self, ticker):
        """Check if stock is in a pullback within an uptrend (buy the dip)"""
        if ticker not in self.price_history:
            return False
        prices = self.price_history[ticker]
        if len(prices) < 30:
        ...
        # Stock pulled back from recent high but not crashed
        pullback = (high_20d - current) / high_20d
        return 0.03 <= pullback <= 0.12  # 3-12% pullback is ideal
    ...
    def on_data(self, data):
        ...
        for ticker in self.symbols.keys():
            ...
            if symbol in data and data[symbol] is not None:
                ...
                # Track price history for 52-week high
                self.price_history[ticker].append(price)
                if len(self.price_history[ticker]) > 260:
                    self.price_history[ticker] = self.price_history[ticker][-260:]
    ...
    def check_exits(self):
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            # Exit if stock drops too far from 52-week high (breakdown)
            if not self.is_near_52_week_high(ticker):
                if pnl < 0.05:  # Only exit if not profitable
                    should_exit = True
    ...
    def rebalance(self):
        ...
        for ticker in self.symbols.keys():
            ...
            # KEY FILTER: Must be near 52-week high
            if not self.is_near_52_week_high(ticker):
                continue
            ...
            # Bonus points for pullback entries
            is_pullback = self.is_pullback_in_uptrend(ticker)
            ...
            if ticker in self.adx_ind and self.adx_ind[ticker].is_ready:
                ...
                if is_pullback:
                    score *= 1.2  # Bonus for pullback entries
            else:
