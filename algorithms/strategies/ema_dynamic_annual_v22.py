...
class EMADynamicAnnualV22(QCAlgorithm):
    """
    V22: QUARTERLY UNIVERSE REFRESH

    Changes:
    - Universe refresh quarterly (Jan, Apr, Jul, Oct) instead of annually
    - Catches momentum rotations faster
    - 5 positions at 20%
    - 15/75 EMAs (faster than 20/100)
    - Top 30 stocks from top quartile volatility
    ...
    def initialize(self):
        ...
        self.last_refresh_quarter = None  # Track quarter instead of year
        ...
        self.set_warm_up(85, Resolution.DAILY)

    def get_quarter(self, month):
        """Return quarter number (1-4) from month"""
        return (month - 1) // 3 + 1

    def coarse_filter(self, coarse):
        current_year = self.time.year
        current_quarter = self.get_quarter(self.time.month)
        current_key = f"{current_year}Q{current_quarter}"

        # Only refresh in first month of quarter (Jan, Apr, Jul, Oct)
        refresh_months = [1, 4, 7, 10]

        if self.last_refresh_quarter == current_key:
            return list(set(self.universe_symbols) | self.held_symbols)

        if self.time.month not in refresh_months and self.last_refresh_quarter is not None:
            return list(set(self.universe_symbols) | self.held_symbols)

        self.debug(f"QUARTERLY REFRESH: {current_key}")
    ...
    def fine_filter(self, fine):
        ...
        self.last_refresh_quarter = current_key

        self.debug(f"V22 Universe: {len(top_stocks)} stocks for {current_key}")
