...
class EMADynamicAnnualV64Test(QCAlgorithm):
    """
    V64: Test V58's EXACT fine_filter to isolate the difference

    Uses V58's fine_filter with regime branching but starts in 2020
    where regime = SPECULATIVE_BULL throughout.

    Expected: Should match V30/V63 at 28.58% if fine_filter is equivalent
    """
    ...
    def fine_filter(self, fine):
        ...
        # V58 excluded sectors
        excluded_sectors = [103, 104, 207]

        stock_metrics = []

        # V58's inline filtering loop
        for x in fine:
            try:
                # Sector filter
                ...
                # ====== REGIME-SPECIFIC FILTERS (from V58) ======
        ...
        # ====== V58's REGIME-SPECIFIC VOLATILITY FILTER ======
