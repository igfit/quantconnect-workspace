...
class EMADynamicAnnualV50(QCAlgorithm):
    """
    V50: REGIME-ADAPTIVE META STRATEGY

    Problem: V30 works in bubbles (2020-2024) but loses in normal markets (2015-2020).
    Solution: Detect market regime and adapt strategy parameters.

    REGIME DETECTION (SPY-based):
    - BULL:    SPY > 200 EMA AND 50 EMA > 200 EMA (golden cross)
    - BEAR:    SPY < 200 EMA
    - NEUTRAL: Otherwise

    STRATEGY BY REGIME:
    - BULL:    Aggressive (top 10% vol, 5 pos @ 20%, 30% trail)
    - NEUTRAL: Moderate (top 20% vol, 4 pos @ 15%, 25% trail)
    - BEAR:    Cash (no positions)

    Expected: Lower CAGR than V30 in bull-only periods, but positive returns across all regimes.
    """

    def initialize(self):
        self.set_start_date(2015, 1, 1)  # Full 10-year test
        ...
        # EMA settings for signals
        self.ema_fast = 20
        self.ema_slow = 100

        # Regime-dependent parameters (set dynamically)
        ...
        self.set_warm_up(220, Resolution.DAILY)  # Longer warmup for 200 EMA
    ...
    def update_regime(self):
        """Detect market regime based on SPY EMAs"""
        if not self.spy_ema50.is_ready or not self.spy_ema200.is_ready:
            return

        spy_price = self.securities[self.spy].price
        ema50 = self.spy_ema50.current.value
        ema200 = self.spy_ema200.current.value

        old_regime = self.current_regime

        # Regime detection logic
        if spy_price > ema200 and ema50 > ema200:
            self.current_regime = "BULL"
            self.vol_percentile = 90      # Top 10% vol
            self.max_positions = 5
            self.weight_per_stock = 0.20  # 100% exposure
            self.trailing_stop_pct = 0.30
        elif spy_price < ema200:
            self.current_regime = "BEAR"
            self.vol_percentile = 80
            self.max_positions = 0        # No positions
            self.weight_per_stock = 0
            self.trailing_stop_pct = 0.20
        else:
            self.current_regime = "NEUTRAL"
            self.vol_percentile = 80      # Top 20% vol
            self.max_positions = 4
        ...
        if old_regime != self.current_regime:
            self.debug(f"REGIME CHANGE: {old_regime} -> {self.current_regime} | SPY={spy_price:.2f} EMA50={ema50:.2f} EMA200={ema200:.2f}")

            # If switching to BEAR, liquidate all positions
            if self.current_regime == "BEAR" and self.held_symbols:
