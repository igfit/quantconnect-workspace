...
class IndicatorMomentumRank(QCAlgorithm):
    """
    Momentum Ranking Strategy (Top 5 Concentrated)

    CONCEPT:
    - Rank all 25 stocks by 3-month momentum (ROC)
    - Only hold TOP 5 stocks with highest momentum
    - Rebalance monthly
    - 20% weight each = concentrated bets

    WHY THIS MIGHT WORK:
    - Momentum factor is well-documented
    - Concentrated bets = capture winners
    - Monthly rebalance = ride trends longer
    """
    ...
    def rebalance(self):
        ...
        self.debug(f"Top {self.top_n}: {top_tickers}")
