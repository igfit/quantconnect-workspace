...
class HardCap8PctStrategy(QCAlgorithm):
    """
    Round 11 Strategy 2: Hard 8% Position Cap

    Key idea: No position can exceed 8% of portfolio.
    - Monthly rebalance sells down winners to cap
    - Prevents any single stock from dominating returns
    - Alpha must come from signal timing, not riding winners

    Same entry/exit logic as TrendStrengthMom, but with hard caps.
    """
    ...
    def initialize(self):
        ...
        self.target_positions = 12   # More positions for diversification
    ...
    def rebalance(self):
        ...
        # HARD CAP: Each position gets max 8%, equal weight otherwise
        base_weight = 1.0 / len(selected)
        weight = min(base_weight, self.max_position_pct)

        # Liquidate all and reallocate (ensures hard cap is enforced)
        self.liquidate()
        ...
        self.debug(f"{self.time.date()}: {len(selected)} positions @ {weight:.1%} each")
