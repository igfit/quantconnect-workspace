...
class EMADynamicAnnualV70Test(QCAlgorithm):
    """
    V70: V68 with ASSERT to verify regime

    If regime is wrong, the backtest will crash - making it obvious.
    """
    ...
    def initialize(self):
        ...
        self.current_regime = "SPECULATIVE_BULL"  # Initialize to correct value!
    ...
    def fine_filter(self, fine):
        ...
        # ASSERT: regime must be SPECULATIVE_BULL for 2020-2025 backtest
        if self.time.year >= 2020:
            if regime != "SPECULATIVE_BULL":
                raise ValueError(f"REGIME ERROR: Expected SPECULATIVE_BULL, got {regime} on {self.time}")

        self.debug(f"Fine filter: regime={regime}, time={self.time}")
        ...
        if regime == "QUALITY_BULL":
            self.debug("Using QUALITY_BULL vol filter")
            if len(vols) >= 10:
                vol_30th = np.percentile(vols, 30)
                vol_70th = np.percentile(vols, 70)
                high_vol = [x for x in stock_metrics if vol_30th <= x[2] <= vol_70th]
            else:
                high_vol = stock_metrics

        elif regime == "SPECULATIVE_BULL":
            self.debug("Using SPECULATIVE_BULL vol filter")
            if len(vols) >= 10:
            ...
            self.debug(f"Using FALLBACK vol filter (regime={regime})")
            high_vol = stock_metrics

        self.debug(f"Selected {len(high_vol)} high-vol stocks from {len(stock_metrics)}")
