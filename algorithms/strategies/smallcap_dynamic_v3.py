...
class SmallCapDynamicV3(QCAlgorithm):
    """
    V3: Aggressive - micro/small cap, high volatility, pure momentum

    Hypothesis: Smaller stocks with momentum have highest upside potential
    """
    ...
    def initialize(self):
        ...
        self.short_lookback = 21              # Faster momentum
        self.long_lookback = 63               # 3 months
        self.beta_lookback = 126
        self.rs_lookback = 14                 # 2-week relative strength

        # Universe: micro/small cap, high volume
        self.min_market_cap = 200_000_000     # $200M (smaller)
        self.max_market_cap = 3_000_000_000   # $3B (tighter cap)
        self.min_price = 5
        self.min_dollar_volume = 15_000_000   # $15M/day (very liquid)
        ...
        self.max_total_exposure = 0.85        # Higher exposure in bull

        self.stop_atr_mult = 2.0              # Tighter stops
        self.trailing_atr_mult = 2.5
        self.trailing_activation = 0.08
        self.vix_entry_threshold = 30         # More tolerant of volatility
        self.vix_exit_threshold = 38
        self.rs_percentile = 0.35             # Top 35% only

        # Rebalance twice per week for faster response
        self.schedule.on(
    ...
    def fine_filter(self, fine):
        ...
        filtered = []
        for x in fine:
            # Tight market cap for micro/small
            if not (self.min_market_cap < x.market_cap < self.max_market_cap):
                continue

            # No sector filter - let momentum decide
            filtered.append(x)

        # Sort by dollar volume (highest liquidity = most institutional interest)
    ...
    def check_exits(self):
        ...
        for ticker in list(self.entry_prices.keys()):
            ...
            # Faster exit for laggards
            if ticker not in leaders and pnl < 0.02:
                should_exit = True
    ...
    def rebalance(self):
        ...
        for symbol in self.universe_symbols:
            ...
            if short_mom <= 0.01:  # Lower threshold
                continue
