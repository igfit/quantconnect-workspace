"""
Base Algorithm Template for Strategy Factory

This template contains ALL safety guards for live trading.
The compiler fills in the placeholders with strategy-specific code.

SAFETY GUARDS IMPLEMENTED:
1. Trade on next-day open (no look-ahead bias)
2. Slippage model (0.1%)
3. Commission model (IBKR: $0.005/share, $1 min)
4. Liquidity filter (min dollar volume)
5. Price filter (min $5)
6. Indicator warmup period
7. Data existence checks
8. Single position per symbol
"""

# This is the template string that will be formatted by the compiler
ALGORITHM_TEMPLATE = '''
...
class {class_name}(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.set_security_initializer(lambda security: security.set_slippage_model(
            ConstantSlippageModel({slippage_percent})
        ))
...
def get_template() -> str:
    """Return the algorithm template string"""
