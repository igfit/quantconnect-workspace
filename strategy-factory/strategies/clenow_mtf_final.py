...
2. ATR-Based Volatility Scaling: Less capital in high-volatility stocks
3. Momentum Acceleration Filter: Momentum must be IMPROVING
4. ATR Trailing Stop: Dynamic stop based on stock's own volatility (2.5x ATR)
5. Sector Diversification: Max 3 positions per sector
6. Defensive Regime Filter: 30% exposure when SPY < 200 SMA
7. Optimal Leverage: 1.9x to hit return target
...
class ClenowMTFFinal(QCAlgorithm):
    ...
    def get_momentum_acceleration(self, symbol):
        """Check if momentum is accelerating (current > 20 days ago)"""
    ...
    def is_weekly_uptrend(self, symbol) -> bool:
        """Multi-timeframe: Price must be above 50-day EMA (≈10 week)"""
    ...
    def is_uptrending(self, symbol) -> bool:
        """Daily trend: Price > 50 SMA > 100 SMA, recent return > -5%"""
    ...
    def get_volatility_weight(self, symbol):
        """Inverse ATR weighting: Lower volatility = higher allocation"""
    ...
    def apply_sector_limits(self, rankings):
        """Enforce max positions per sector for diversification"""
        sector_counts = {}
        filtered = []
    ...
    def daily_risk_check(self):
        """Daily ATR trailing stop and trend break exit"""
        ...
        for symbol in list(self.current_holdings):
            ...
            if self.USE_ATR_TRAILING and symbol in self.stock_atr:
                ...
                if atr.is_ready:
                    ...
                    if current_price < atr_stop:
                        self.log(f"ATR STOP: {symbol.value}")
            ...
            if not self.is_uptrending(symbol):
                ...
                if avg_price > 0 and current_price < avg_price * 0.97:
                    self.log(f"TREND EXIT: {symbol.value}")
                    self.liquidate(symbol)
                    self.current_holdings.discard(symbol)
    ...
    def rebalance(self):
        """Monthly rebalancing with all filters"""
        if self.is_warming_up:
            return

        regime_exposure = self.get_regime_exposure()

        # Go to cash in severe bear market
        ...
        # Log selections
        self.log(f"TOP {self.TOP_N}: {', '.join([s.value for s in top_stocks])}")
        ...
        # Calculate weights with volatility scaling and squared momentum
        weights = []
