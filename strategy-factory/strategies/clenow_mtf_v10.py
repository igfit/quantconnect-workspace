"""
Clenow Momentum v10: Fine-tuned for 30% CAGR, <30% DD

v9 achieved 22% CAGR, 32% DD (slightly over target)
v10 adjustments:
1. Reduce leverage to 1.2x (from 1.3x)
2. Tighter ATR stop: 2.5x ATR (from 3x)
3. More aggressive regime filter: 0.3x in bear (from 0.5x)
"""
...
class ClenowMTFv10(QCAlgorithm):
    ...
    # v10 ADJUSTMENTS
    LEVERAGE = 1.2  # Reduced from 1.3
    MAX_PER_SECTOR = 3
    VOL_SCALE_FACTOR = 0.5
    ATR_TRAILING_MULT = 2.5  # Tighter from 3.0
    BEAR_EXPOSURE = 0.3  # More defensive from 0.5
