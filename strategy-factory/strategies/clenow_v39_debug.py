"""
Clenow v39: Debug Version - with extensive logging

Same as v39 but logs why trades aren't happening.
"""
...
class ClenowDebug(QCAlgorithm):
    ...
    def get_regime_exposure(self) -> float:
        ...
        return 1.0 if spy_price > spy_sma else self.BEAR_EXPOSURE
    ...
    def rebalance(self):
        ...
        spy_price = self.securities[self.spy].price
        spy_sma = self.spy_sma.current.value if self.spy_sma.is_ready else 0

        # Log regime status monthly
        month_key = f"{self.time.year}-{self.time.month}"
        if self.last_log != month_key:
            self.last_log = month_key
            self.log(f"REGIME: SPY={spy_price:.2f} SMA={spy_sma:.2f} exposure={regime}")
        ...
        if len(candidates) < self.TOP_N:
            self.log(f"NO CANDIDATES: uptrending={uptrending_count} mom_passed={mom_passed} candidates={len(candidates)}")
            return
        ...
        self.log(f"REBAL: {', '.join([f'{s.value}' for s, _ in top])}")
