"""
Clenow v51: Momentum Acceleration

Hypothesis: Stocks with ACCELERATING momentum (recent momentum > older momentum)
are more likely to continue. Add momentum acceleration as a scoring factor.

Changes from v39:
- Add momentum acceleration factor: compare 25-day vs 50-day momentum
- Boost score for stocks with accelerating momentum
- Keep all other v39 parameters
"""
...
class ClenowMomentumAccel(QCAlgorithm):
    ...
    ACCEL_LOOKBACK = 25  # Short-term momentum for acceleration calc
    TOP_N = 3
    MIN_MOMENTUM = 45
    MIN_REL_STRENGTH = 12
    MIN_R_SQUARED = 0.50
    ...
    def initialize(self):
        ...
        self.schedule.on(self.date_rules.every(DayOfWeek.MONDAY),
            self.time_rules.after_market_open(self.spy, 30), self.check_rebalance)
        self.schedule.on(self.date_rules.every_day(),
            self.time_rules.after_market_open(self.spy, 60), self.daily_risk_check)
    ...
    def calculate_momentum(self, symbol, lookback=None):
        if lookback is None:
    ...
    def calculate_momentum_acceleration(self, symbol):
        """Return ratio of short-term momentum to long-term momentum."""
        short_mom, _ = self.calculate_momentum(symbol, self.ACCEL_LOOKBACK)
        long_mom, _ = self.calculate_momentum(symbol, self.MOMENTUM_LOOKBACK)
        if short_mom is None or long_mom is None or long_mom <= 0:
            return 1.0
    ...
    def rebalance(self):
        ...
        for symbol in self.stocks:
            ...
            if mom > self.MIN_MOMENTUM:
                ...
                accel = self.calculate_momentum_acceleration(symbol)
                accel_boost = max(0.8, min(1.5, accel))  # Cap between 0.8x and 1.5x
                score = mom * (1 + rel_str / 100) * accel_boost
