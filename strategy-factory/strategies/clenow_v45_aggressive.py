"""
Clenow v45 Aggressive: Push for 20%+ in 2024-2025

More aggressive approach:
1. Higher leverage: 1.5x
2. Shorter momentum lookback: 35 days (faster signals)
3. Weekly rebalancing (faster adaptation)
4. Shorter cooldown: 2 weeks
5. Keep volatility filter but exclude only worst losers
6. Minimum 4 positions for diversification at higher leverage
"""
...
class ClenowAggressiveV45(QCAlgorithm):

    # Faster momentum
    MOMENTUM_LOOKBACK = 35
    TOP_N = 4  # More positions for leverage diversification
    MIN_MOMENTUM = 45  # Lower threshold for shorter lookback
    MIN_REL_STRENGTH = 10
    MIN_R_SQUARED = 0.45

    # Higher leverage
    LEVERAGE = 1.5
    ATR_TRAILING_MULT = 2.8  # Slightly wider for more room
    BEAR_EXPOSURE = 0.0
    MAX_PER_SECTOR = 1
    ...
    # Shorter cooldown for faster re-entry
    COOLDOWN_WEEKS = 2
    ...
    def initialize(self):
        ...
        # Weekly rebalancing for faster adaptation
        self.schedule.on(
    ...
    def daily_risk_check(self):
        ...
        for symbol in list(self.current_holdings):
            ...
            if not self.is_uptrending(symbol):
                ...
                if avg > 0 and price < avg * 0.92:  # Tighter stop
                    self.liquidate(symbol)
                    self.current_holdings.discard(symbol)
                    if symbol in self.position_peaks:
                        del self.position_peaks[symbol]
        ...
        if len(candidates) < self.TOP_N:
            for symbol in self.stocks:
                ...
                if self.is_in_cooldown(symbol):
                    continue
                mom, rsq = self.calculate_momentum(symbol)
                if mom is not None and mom > 20 and rsq is not None and rsq > 0.35:
                    rel_str = self.calculate_relative_strength(symbol)
