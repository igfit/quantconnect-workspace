"""
Clenow v55: Hybrid Best Practices

Combines best insights from all iterations:
- v39 baseline parameters (proven optimal)
- Keep winners unless significantly better candidates appear
- Slightly more positions (4 instead of 3) for diversification
- But allow 2 per sector (since tech/semi dominate)
"""
...
class ClenowHybrid(QCAlgorithm):
    ...
    TOP_N = 4  # Slightly more positions
    MIN_MOMENTUM = 45
    MIN_REL_STRENGTH = 12
    MIN_R_SQUARED = 0.50

    LEVERAGE = 1.0
    ATR_TRAILING_MULT = 2.5  # Slightly looser stop
    BEAR_EXPOSURE = 0.0
    MAX_PER_SECTOR = 2  # Allow 2 per sector

    ATR_PERIOD = 20
    USE_ATR_TRAILING = True
    MAX_POSITION_SIZE = 0.30  # Smaller per position

    # Turnover threshold - only replace if new candidate is 20% better
    TURNOVER_THRESHOLD = 1.20
    ...
    def initialize(self):
        ...
        self.current_holdings = {}  # Now tracks scores too
        self.position_peaks = {}
    ...
    def rebalance(self):
        ...
        new_holdings = {s: score for s, score in top}

        # Only sell if current holding isn't in top or new candidate is significantly better
        for symbol in list(self.current_holdings.keys()):
