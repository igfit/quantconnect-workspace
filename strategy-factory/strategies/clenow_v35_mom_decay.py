"""
Clenow v35: Hyper-Beta with Momentum Decay Exit

Based on v29 (27.9% CAGR, 37% DD)

Innovation: Exit when momentum DECELERATES, not just when price breaks trend.
If current momentum < momentum 10 days ago by >20%, exit early.

This should catch trend reversals earlier than ATR stops.
"""
...
class ClenowMomDecay(QCAlgorithm):
    ...
    MOM_DECAY_LOOKBACK = 10
    MOM_DECAY_THRESHOLD = 0.20  # Exit if momentum drops 20% from recent
    ...
    def initialize(self):
        ...
        self.stock_sma = {}
        self.stock_atr = {}
        self.stock_mom_history = {}  # Track momentum over time
    ...
    def update_momentum_history(self, symbol):
        """Track momentum over time for decay detection"""
        mom, _ = self.calculate_momentum(symbol)
        if mom is not None:
            self.stock_mom_history[symbol].append(mom)
            # Keep only recent history
            if len(self.stock_mom_history[symbol]) > 20:
                self.stock_mom_history[symbol] = self.stock_mom_history[symbol][-20:]

    def check_momentum_decay(self, symbol) -> bool:
        """Return True if momentum has decayed significantly"""
        history = self.stock_mom_history.get(symbol, [])
        if len(history) < self.MOM_DECAY_LOOKBACK:
            return False

        current_mom = history[-1]
        past_mom = history[-self.MOM_DECAY_LOOKBACK]

        if past_mom <= 0:
            return False

        decay = (past_mom - current_mom) / past_mom
        return decay > self.MOM_DECAY_THRESHOLD
    ...
    def daily_risk_check(self):
        ...
        # Update momentum history for all stocks
        for symbol in self.stocks:
            self.update_momentum_history(symbol)
        ...
        for symbol in list(self.current_holdings):
            ...
            if self.check_momentum_decay(symbol):
                self.log(f"MOM DECAY EXIT: {symbol.value}")
    ...
    def rebalance(self):
        ...
        self.log(f"MOM DECAY: {', '.join([f'{s.value}' for s, _ in top])}")
