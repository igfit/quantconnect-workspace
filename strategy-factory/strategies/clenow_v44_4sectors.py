"""
Clenow v44: 4 positions, 1 per sector (4 different sectors)

Hypothesis: v39's forced sector diversification (1 per sector) helps CAGR
by catching different momentum waves. Let's try 4 positions across 4 sectors.
"""
...
class Clenow4Sectors(QCAlgorithm):

    MOMENTUM_LOOKBACK = 50
    TOP_N = 4  # 4 positions
    MIN_MOMENTUM = 40  # Lower to find candidates in 4 sectors
    MIN_REL_STRENGTH = 10
    MIN_R_SQUARED = 0.50
    ...
    BEAR_EXPOSURE = 0.0
    MAX_PER_SECTOR = 1  # Force 4 different sectors
    ...
    def rebalance(self):
        ...
        self.log(f"4SEC: {', '.join([f'{s.value}({self.get_sector(s)})' for s, _ in top])}")
