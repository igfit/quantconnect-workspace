"""
Clenow v40: More Aggressive with Broad Universe

Based on v39 (23.6% CAGR, 23.2% DD)
Changes:
- 4 positions (more opportunities)
- Lower rel strength threshold (more entries)
- Allow 2 per sector
- Higher momentum threshold for quality
"""
...
class ClenowAggressive(QCAlgorithm):

    MOMENTUM_LOOKBACK = 50
    TOP_N = 4
    MIN_MOMENTUM = 55  # Higher quality
    MIN_REL_STRENGTH = 8  # Lower threshold
    MIN_R_SQUARED = 0.50

    LEVERAGE = 1.0
    ATR_TRAILING_MULT = 2.5  # Slightly looser
    BEAR_EXPOSURE = 0.0
    MAX_PER_SECTOR = 2
    ...
    def rebalance(self):
        ...
        self.log(f"AGG: {', '.join([f'{s.value}' for s, _ in top])}")
