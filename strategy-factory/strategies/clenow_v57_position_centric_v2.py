"""
Clenow v57: Position-Centric v2

Fixes from v56:
- Much stricter entry (momentum > 70, not 50)
- Require stock at 20-day high for entry
- Looser exits (let winners run)
...
class ClenowPositionCentricV2(QCAlgorithm):

    # STRICTER entry thresholds
    MOMENTUM_LOOKBACK = 50
    MIN_MOMENTUM = 70  # Much higher bar
    MIN_REL_STRENGTH = 20  # Must strongly beat SPY
    MIN_R_SQUARED = 0.55

    # LOOSER exit thresholds
    EXIT_MOMENTUM = 10  # Only exit when momentum really dies
    ATR_TRAILING_MULT = 3.0  # Looser trailing stop

    # Position management
    MAX_POSITIONS = 3  # Fewer positions
    POSITION_SIZE = 0.30  # Larger positions
    MAX_PER_SECTOR = 1  # Strict diversification
    ...
    def initialize(self):
        ...
        self.stock_sma = {}
        self.stock_atr = {}
        self.stock_high = {}  # Track 20-day highs
        for symbol in self.stocks:
            ...
            self.stock_high[symbol] = self.max(symbol, 20, Resolution.DAILY)
    ...
    def is_at_high(self, symbol) -> bool:
        """Check if price is within 2% of 20-day high."""
        if symbol not in self.stock_high:
            return False
        high = self.stock_high[symbol]
        if not high.is_ready:
            return False
        price = self.securities[symbol].price
        return price >= high.current.value * 0.98
    ...
    def check_entry_signal(self, symbol) -> bool:
        # Must be in uptrend AND near highs
        if not self.is_uptrending(symbol):
            return False
        if not self.is_at_high(symbol):
            return False

        # Check momentum - high bar
        ...
        # Check relative strength - must strongly beat SPY
    ...
    def check_exit_signal(self, symbol) -> str:
        ...
        # Momentum breakdown - only exit when momentum really dies
