"""
Clenow v54: Volatility Targeting

Hypothesis: Scale position sizes inversely with market volatility.
In low-vol periods, take more risk. In high-vol periods, reduce exposure.
This could boost returns during calm bull markets while protecting in corrections.

Changes from v39:
- Add VIX-based volatility scaling
- Target 15% annualized volatility
- Scale positions up/down based on realized volatility
"""
...
class ClenowVolTargeting(QCAlgorithm):
    ...
    MAX_POSITION_SIZE = 0.50  # Allow higher position sizes

    # Volatility targeting parameters
    TARGET_VOL = 0.15  # 15% target volatility
    VOL_LOOKBACK = 20  # Days for realized vol calculation
    VOL_SCALE_MAX = 1.5  # Max scale-up in low vol
    VOL_SCALE_MIN = 0.5  # Min scale-down in high vol
    ...
    def calculate_vol_scalar(self):
        """Calculate position scaling based on realized volatility."""
        spy_hist = self.history(self.spy, self.VOL_LOOKBACK + 1, Resolution.DAILY)
        if spy_hist.empty or len(spy_hist) < self.VOL_LOOKBACK:
            return 1.0
        try:
            ...
            return max(self.VOL_SCALE_MIN, min(self.VOL_SCALE_MAX, scalar))
        except:
            return 1.0
    ...
    def rebalance(self):
        ...
        vol_scalar = self.calculate_vol_scalar()
        weight = (self.LEVERAGE * regime * vol_scalar) / self.TOP_N
