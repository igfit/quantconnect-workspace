"""
Clenow v41: Concentrated with Weekly Rebalancing

Back to 2 positions for higher CAGR, but with:
- Weekly rebalancing to catch momentum faster
...
class ClenowConcentrated(QCAlgorithm):
    ...
    TOP_N = 2  # Concentrated
    MIN_MOMENTUM = 60  # High quality
    MIN_REL_STRENGTH = 15
    MIN_R_SQUARED = 0.55
    ...
    def rebalance(self):
        ...
        self.log(f"CONC: {', '.join([f'{s.value}({self.get_sector(s)})' for s, _ in top])}")
