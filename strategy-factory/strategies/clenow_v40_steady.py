"""
Clenow v40 Steady: Improved v39 for more consistent returns

Changes from v39:
1. Wider ATR stop: 2.3x → 3.0x (reduce whipsaw)
2. More positions: 3 → 5 (better diversification)
3. Monthly rebalancing instead of bi-weekly (reduce turnover)
4. Re-entry cooldown: 4 weeks after stop-out (avoid whipsaw)
5. Volatility filter: Skip stocks with ATR% > 4% (too volatile)
6. Lower position size cap: 40% → 25% (more even distribution)

Target: Steadier returns with lower drawdown, accepting slightly lower CAGR
"""
...
class ClenowSteadyV40(QCAlgorithm):

    # Core momentum parameters
    MOMENTUM_LOOKBACK = 50
    TOP_N = 5  # Increased from 3 for diversification
    MIN_MOMENTUM = 45
    MIN_REL_STRENGTH = 12
    MIN_R_SQUARED = 0.50
    ...
    # Risk parameters
    LEVERAGE = 1.0
    ATR_TRAILING_MULT = 3.0  # Wider stop (was 2.3)
    BEAR_EXPOSURE = 0.0
    MAX_PER_SECTOR = 2  # Allow 2 per sector with 5 positions

    # Position management
    ATR_PERIOD = 20
    USE_ATR_TRAILING = True
    MAX_POSITION_SIZE = 0.25  # Lower cap for more even distribution

    # Volatility filter
    MAX_ATR_PERCENT = 0.04  # Skip stocks with >4% daily ATR

    # Cooldown to prevent whipsaw
    COOLDOWN_WEEKS = 4
    ...
    def initialize(self):
        ...
        # Cooldown tracking: symbol -> week when cooldown expires
        self.cooldown_until = {}
    ...
    def is_too_volatile(self, symbol) -> bool:
        """Check if stock is too volatile (ATR% > threshold)"""
        if symbol not in self.stock_atr:
            return True
    ...
    def is_in_cooldown(self, symbol) -> bool:
        """Check if stock is in cooldown period after stop-out"""
            ...
            if self.USE_ATR_TRAILING and symbol in self.stock_atr:
                ...
                if atr.is_ready:
                    ...
                    if price < stop:
                        ...
                        self.log(f"STOP: {symbol.value} - cooldown until week {self.cooldown_until[symbol]}")
                        continue
    ...
    def rebalance(self):
        ...
        self.log(f"REBAL: {', '.join([f'{s.value}({self.get_sector(s)})' for s, _ in top])}")
