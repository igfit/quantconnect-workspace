"""
Clenow v37: Balanced - Between v29 and v36

v29: 27.9% CAGR, 37% DD (2 positions, hyper-beta, bi-weekly)
v36: 17.7% CAGR, 16.7% DD (3 positions, dual momentum, bi-weekly)

v37: Try to get 25%+ CAGR with <30% DD by:
- 2 positions (more concentration)
- Dual momentum filter (better quality)
- Slightly looser ATR stop (2.5x)
- Hyper-beta universe
"""
...
class ClenowBalanced(QCAlgorithm):

    MOMENTUM_LOOKBACK = 50  # Middle ground
    TOP_N = 2  # Concentration from v29
    MIN_MOMENTUM = 50
    MIN_REL_STRENGTH = 15  # Dual momentum
    MIN_R_SQUARED = 0.50

    LEVERAGE = 1.0
    ATR_TRAILING_MULT = 2.5  # From v29
    BEAR_EXPOSURE = 0.0
    ...
    def rebalance(self):
        ...
        self.log(f"BALANCED: {', '.join([f'{s.value}' for s, _ in top])}")
