...
class Rsi2MeanReversion(QCAlgorithm):
    ...
    Buy oversold conditions (RSI-2 < 25) in stocks above 200 EMA, exit when overbought

    Rationale: Short-term panic creates buying opportunities in quality uptrends. 2-period RSI is extremely sensitive to short-term oversold conditions. The 200 EMA filter ensures we only buy dips in uptrends, not falling knives. Edge exists because buying when everyone is selling is psychologically difficult. Threshold relaxed from < 10 to < 25 for more signals.

    Generated by Strategy Factory
    Strategy ID: rsi2_mean_rev
    """
    ...
    def initialize(self):
        ...
        self.symbols = []
        # Static universe
        for ticker in ["AAPL", "MSFT", "GOOGL", "NVDA", "AMZN", "META"]:
        ...
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['ema_200'] = self.ema(symbol, 200, Resolution.DAILY)
    ...
    def _check_entry_conditions(self, symbol) -> bool:
        ...
        """
        return (self._get_indicator_value(symbol, "rsi_2") < 25) and (self._get_indicator_value(symbol, "price") > self._get_indicator_value(symbol, "ema_200"))
    ...
    def _check_exit_conditions(self, symbol) -> bool:
        ...
        GENERATED CODE - DO NOT EDIT
        """
        return (self._get_indicator_value(symbol, "rsi_2") > 65)
