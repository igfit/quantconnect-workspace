...
class HighBetaRsiBounce(QCAlgorithm):
    """
    High Beta RSI Bounce

    Buy oversold high-beta stocks for quick bounces

    Rationale: High-beta stocks often overshoot on the downside. RSI oversold + uptrend filter catches quality dips. Quick exit targets capture the bounce.

    Generated by Strategy Factory
    Strategy ID: d131788c
    """
    ...
    def initialize(self):
        ...
        self.set_security_initializer(lambda security: security.set_fee_model(
        ...
        self.symbols = []
        # Static universe
        for ticker in ["TSLA", "NVDA", "AMD", "COIN", "SQ", "SHOP"]:
        ...
        for symbol in self.symbols:
            ...
            self.indicators[symbol]['rsi_5'] = self.rsi(symbol, 5, Resolution.DAILY)
        
        # Indicator: sma_100 (SMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['sma_100'] = self.sma(symbol, 100, Resolution.DAILY)
    ...
    def _check_entry_conditions(self, symbol) -> bool:
        ...
        """
        return (self._get_indicator_value(symbol, "rsi_5") < 20) and (self._get_indicator_value(symbol, "price") > self._get_indicator_value(symbol, "sma_100"))
    ...
    def _check_exit_conditions(self, symbol) -> bool:
        ...
        GENERATED CODE - DO NOT EDIT
        """
        return (self._get_indicator_value(symbol, "rsi_5") > 60)
    ...
    def _get_prev_value(self, symbol, name) -> float:
        """Get previous indicator value for crossover detection"""
        prev = self.prev_indicator_values.get(symbol, {})
        return prev.get(name, 0.0)
