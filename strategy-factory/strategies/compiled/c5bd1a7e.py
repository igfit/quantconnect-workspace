...
class MaCrossoverMomentum(QCAlgorithm):
    """
    MA Crossover Momentum

    Buy when fast MA crosses above slow MA, sell on reverse

    Rationale: Moving average crossovers capture trend changes. The lag provides confirmation and reduces whipsaws. Works best in trending markets.

    Generated by Strategy Factory
    Strategy ID: c5bd1a7e
    """
    ...
    def initialize(self):
        ...
        # Indicator: fast_sma (SMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['fast_sma'] = self.sma(symbol, 20, Resolution.DAILY)
        
        # Indicator: slow_sma (SMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['slow_sma'] = self.sma(symbol, 50, Resolution.DAILY)
    ...
    def _check_entry_conditions(self, symbol) -> bool:
        ...
        """
        return (self._crosses_above(symbol, "fast_sma", "slow_sma"))
    ...
    def _check_exit_conditions(self, symbol) -> bool:
        ...
        GENERATED CODE - DO NOT EDIT
        """
        return (self._crosses_below(symbol, "fast_sma", "slow_sma"))
