...
class DualMomentumSpy(QCAlgorithm):
    """
    Dual Momentum SPY

    Hold SPY when it shows positive absolute momentum (12-month ROC > 0), otherwise stay out

    Rationale: Based on Gary Antonacci's research - combines relative and absolute momentum. First place NAAIM award 2012, backtested to 215 years. During 2008: strategy UP 20%, S&P DOWN 40%. This simplified version uses absolute momentum only - stay invested when trend is positive, exit when negative. Avoids major drawdowns while capturing bull market gains.
    ...
    def initialize(self):
        ...
        # Indicator: roc_252 (ROC)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['roc_252'] = self.roc(symbol, 252, Resolution.DAILY)
        
        # Indicator: sma_200 (SMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['sma_200'] = self.sma(symbol, 200, Resolution.DAILY)
    ...
    def _check_entry_conditions(self, symbol) -> bool:
        ...
        """
        return (self._get_indicator_value(symbol, "roc_252") > 0) and (self._get_indicator_value(symbol, "price") > self._get_indicator_value(symbol, "sma_200"))
    ...
    def _check_exit_conditions(self, symbol) -> bool:
        ...
        GENERATED CODE - DO NOT EDIT
        """
        return (self._get_indicator_value(symbol, "roc_252") < 0) or (self._get_indicator_value(symbol, "price") < self._get_indicator_value(symbol, "sma_200"))
