...
class SectorEtfRotation(QCAlgorithm):
    """
    Sector ETF Rotation

    Buy sector ETFs with positive 3-month momentum, exit when momentum turns negative

    Rationale: Sectors rotate predictably with economic cycles. ETFs provide diversification within sectors, high liquidity, and lower volatility than individual stocks. 3-month ROC captures sector leadership changes. Simplified to single condition for more signals.

    Generated by Strategy Factory
    Strategy ID: sector_rotation
    """
    ...
    def initialize(self):
        ...
        self.symbols = []
        # Static universe
        for ticker in ["XLK", "XLF", "XLE", "XLV", "XLY", "XLP", "XLI", "XLU"]:
        ...
        # Indicator: roc_63 (ROC)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['roc_63'] = self.roc(symbol, 63, Resolution.DAILY)
