...
class EmaMacdTrend(QCAlgorithm):
    """
    EMA + MACD Trend

    Follow EMA trend with MACD momentum confirmation

    Rationale: EMA responds faster to price changes than SMA. MACD histogram shows momentum shifts. Double confirmation reduces false signals.

    Generated by Strategy Factory
    Strategy ID: fb619578
    """
    ...
    def initialize(self):
        ...
        self.symbols = []
        # Static universe
        for ticker in ["QQQ", "SPY", "AAPL", "NVDA", "TSLA", "AMZN"]:
        ...
        # Indicator: ema_21 (EMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['ema_21'] = self.ema(symbol, 21, Resolution.DAILY)
        
        # Indicator: macd (MACD)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['macd'] = self.macd(symbol, 12, 26, 9, Resolution.DAILY)
    ...
    def _check_entry_conditions(self, symbol) -> bool:
        ...
        """
        return (self._get_indicator_value(symbol, "price") > self._get_indicator_value(symbol, "ema_21"))
