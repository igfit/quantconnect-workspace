...
class MonthlyRocMomentum(QCAlgorithm):
    """
    Monthly ROC Momentum

    Buy stocks with positive 6-month momentum (ROC > 5%), exit when momentum turns negative

    Rationale: Institutional flows take weeks to complete, creating persistent momentum at monthly timeframes. Winners attract more attention, more buying, more winning. Using ROC(126) for 6-month momentum. Universe of high-quality large caps ensures liquidity. Lower threshold (5%) for more signals.

    Generated by Strategy Factory
    Strategy ID: monthly_roc_mom
    """
    ...
    def initialize(self):
        ...
        # Indicator: roc_126 (ROC)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['roc_126'] = self.roc(symbol, 126, Resolution.DAILY)
    ...
    def _check_entry_conditions(self, symbol) -> bool:
        ...
        """
        return (self._get_indicator_value(symbol, "roc_126") > 5) and (self._get_indicator_value(symbol, "price") > self._get_indicator_value(symbol, "sma_50"))
    ...
    def _check_exit_conditions(self, symbol) -> bool:
        ...
        GENERATED CODE - DO NOT EDIT
        """
        return (self._get_indicator_value(symbol, "roc_126") < -5) or (self._get_indicator_value(symbol, "price") < self._get_indicator_value(symbol, "sma_50"))
