...
class EmaCrossoverFast(QCAlgorithm):
    """
    EMA Crossover Fast

    Fast EMA crossover strategy (12/26) for capturing shorter-term trends

    Rationale: EMA responds faster to price changes than SMA. 12/26 is the classic MACD EMA combination, proven to capture trend changes. Faster signals than Golden Cross but more whipsaws. Works because trends at all timeframes tend to persist.

    Generated by Strategy Factory
    Strategy ID: ema_cross_fast
    """
    ...
    def initialize(self):
        ...
        self.symbols = []
        # Static universe
        for ticker in ["QQQ", "SPY", "NVDA", "AAPL", "TSLA", "AMD"]:
        ...
        # Indicator: ema_12 (EMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['ema_12'] = self.ema(symbol, 12, Resolution.DAILY)
        
        # Indicator: ema_26 (EMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['ema_26'] = self.ema(symbol, 26, Resolution.DAILY)
    ...
    def _check_entry_conditions(self, symbol) -> bool:
        ...
        """
        return (self._crosses_above(symbol, "ema_12", "ema_26"))
    ...
    def _check_exit_conditions(self, symbol) -> bool:
        ...
        GENERATED CODE - DO NOT EDIT
        """
        return (self._crosses_below(symbol, "ema_12", "ema_26"))
