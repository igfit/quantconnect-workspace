...
class TurtleBreakout(QCAlgorithm):
    """
    Turtle Breakout

    Buy when price breaks above 20-day SMA (proxy for Donchian high), exit when drops below 10-day SMA

    Rationale: Made the Turtle Traders famous - original strategy returned 80%+ annually in 1980s. New highs signal strong momentum that tends to continue. The asymmetric exit (faster exit than entry) protects profits and lets winners run. Using SMA crossovers as proxy for Donchian channel breakouts. Low win rate (40-50%) but large winners make the system profitable.

    Generated by Strategy Factory
    Strategy ID: turtle_breakout
    """
    ...
    def initialize(self):
        ...
        # Indicator: sma_20 (SMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['sma_20'] = self.sma(symbol, 20, Resolution.DAILY)
        
        # Indicator: sma_10 (SMA)
        for symbol in self.symbols:
            if symbol not in self.indicators:
                self.indicators[symbol] = {}
            self.indicators[symbol]['sma_10'] = self.sma(symbol, 10, Resolution.DAILY)
    ...
    def _check_entry_conditions(self, symbol) -> bool:
        ...
        """
        return (self._crosses_above(symbol, "price", "sma_20"))
    ...
    def _check_exit_conditions(self, symbol) -> bool:
        ...
        GENERATED CODE - DO NOT EDIT
        """
        return (self._crosses_below(symbol, "price", "sma_10"))
