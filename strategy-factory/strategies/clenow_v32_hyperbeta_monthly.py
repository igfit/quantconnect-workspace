"""
Clenow v32: Hyper-Beta Monthly Rebalance

Based on v29 (27.9% CAGR, 37% DD) - trying to reduce DD with monthly rebalancing

Changes from v29:
1. Monthly rebalancing (less churn during volatile periods)
2. 2.3x ATR trailing stop (compromise between v29's 2.5 and v31's 2.0)
3. Same universe and concentration
"""
...
class ClenowHyperBetaMonthly(QCAlgorithm):

    MOMENTUM_LOOKBACK = 42
    TOP_N = 2  # Same concentration as v29
    MIN_MOMENTUM = 40
    MIN_R_SQUARED = 0.50

    LEVERAGE = 1.0
    ATR_TRAILING_MULT = 2.3  # Compromise (v29 was 2.5, v31 was 2.0)
    BEAR_EXPOSURE = 0.0
    ...
    def initialize(self):
        ...
        # MONTHLY rebalancing (vs bi-weekly in v29)
        self.schedule.on(
    ...
    def rebalance(self):
        ...
        self.log(f"MONTHLY: {', '.join([f'{s.value}' for s, _ in top])}")
