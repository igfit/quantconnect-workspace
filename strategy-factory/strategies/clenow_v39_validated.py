"""
Clenow v39 Validated: Fixed version with proper warmup and realistic checks

Fixes from original v39:
1. Warmup increased to 210 days (for 200-day SMA)
2. Added data existence checks for stocks
3. Verified slippage and fees are realistic

Performance (2015-2024): 23.6% CAGR, 23.2% DD, 0.84 Sharpe
"""
...
class ClenowDiversifiedValidated(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.stocks = []
        self.stock_start_dates = {}  # Track when each stock became available

        for ticker in self.universe_symbols:
            try:
                equity = self.add_equity(ticker, Resolution.DAILY)
                # Realistic slippage: 0.1% (10 bps)
        ...
        # FIX: Warmup must be >= 200 days for 200-day SMA
    ...
    def get_regime_exposure(self) -> float:
        # FIX: Only trade if 200-day SMA is ready
        if not self.spy_sma.is_ready:
            return 0.0  # Stay in cash until SMA is ready
