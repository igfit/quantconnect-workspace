"""
Clenow v41 Balanced: More balanced improvement over v39

Changes from v39 (more moderate than v40):
1. ATR stop: 2.3x → 2.6x (slightly wider)
2. Positions: 3 → 4 (modest diversification)
3. Keep bi-weekly rebalancing
4. Re-entry cooldown: 3 weeks after stop-out
5. Volatility filter: ATR% > 5% (only exclude very volatile)
6. Position size cap: 30% (was 40%)

Goal: Reduce whipsaw while maintaining good returns
"""
...
class ClenowBalancedV41(QCAlgorithm):

    # Core momentum parameters
    MOMENTUM_LOOKBACK = 50
    TOP_N = 4  # Increased from 3
    MIN_MOMENTUM = 45
    MIN_REL_STRENGTH = 12
    MIN_R_SQUARED = 0.50
    ...
    # Risk parameters
    LEVERAGE = 1.0
    ATR_TRAILING_MULT = 2.6  # Slightly wider (was 2.3)
    BEAR_EXPOSURE = 0.0
    MAX_PER_SECTOR = 1  # Keep strict sector diversification

    # Position management
    ATR_PERIOD = 20
    USE_ATR_TRAILING = True
    MAX_POSITION_SIZE = 0.30  # Lower cap

    # Volatility filter - only exclude very volatile stocks
    MAX_ATR_PERCENT = 0.05  # 5% daily ATR threshold
