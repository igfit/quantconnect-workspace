"""
Clenow v29: Hyper-Beta with Bi-Weekly Rebalancing

Hypothesis: To get 30%+ CAGR without leverage, need:
1. ONLY the highest-beta stocks (no AAPL/MSFT ballast)
2. Faster rebalancing to catch momentum shifts
3. More concentrated positions (2-3 stocks)
4. Aggressive exits

Universe: Only stocks with historical beta > 1.5
"""
...
class ClenowHyperBeta(QCAlgorithm):

    MOMENTUM_LOOKBACK = 42  # Shorter for faster signals
    TOP_N = 2  # VERY concentrated
    MIN_MOMENTUM = 40
    MIN_R_SQUARED = 0.50
    ...
    def initialize(self):
        ...
        # HYPER-BETA universe - only the most volatile growth stocks
        self.universe_symbols = [
            "NVDA", "TSLA", "AMD", "META", "NFLX",  # Core high-beta
            "MU", "AVGO", "AMAT", "LRCX",  # Semi volatility
            "CRM", "ADBE", "NOW",  # Cloud/SaaS
            "COIN", "SQ", "SHOP",  # Fintech/growth
        ]
    ...
    def daily_risk_check(self):
        ...
        for symbol in list(self.current_holdings):
            ...
            if not self.is_uptrending(symbol):
                ...
                if avg > 0 and price < avg * 0.93:  # Tighter exit
                    self.liquidate(symbol)
                    self.current_holdings.discard(symbol)
    ...
    def rebalance(self):
        ...
        for symbol in self.stocks:
            ...
            if rel_str is None or rel_str < 10:  # Must beat SPY by 10%+
                continue
        ...
        self.log(f"HYPER: {', '.join([f'{s.value}' for s, _ in top])}")
