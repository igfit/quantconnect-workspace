"""
Clenow v43 Adaptive: Optimized for faster-rotating markets (2024-2025)

Key changes to fix 2024-2025 underperformance:
1. Shorter momentum lookback: 50 → 35 days (faster adaptation)
2. Faster regime filter: 200 SMA → 100 SMA (quicker bear detection)
3. Tighter volatility filter: 5% → 3.5% ATR (avoid whipsaw stocks)
4. Longer cooldown: 3 → 5 weeks (prevent repeated losses)
5. Exclude worst performers: Remove ILMN, SBUX from universe
6. Higher leverage: 1.4x to compensate for tighter filters

Target: 20-30% in 2024-2025 specifically
"""
...
class ClenowAdaptiveV43(QCAlgorithm):

    # Faster momentum parameters
    MOMENTUM_LOOKBACK = 35  # Shorter lookback (was 50)
    TOP_N = 3
    MIN_MOMENTUM = 50  # Slightly lower threshold for shorter lookback
    MIN_REL_STRENGTH = 12
    MIN_R_SQUARED = 0.50

    # Risk parameters
    LEVERAGE = 1.4  # Higher leverage to compensate
    ...
    # Tighter volatility filter
    MAX_ATR_PERCENT = 0.035  # 3.5% (was 5%)

    # Longer cooldown
    COOLDOWN_WEEKS = 5  # Was 3

    # Faster regime filter
    REGIME_SMA_PERIOD = 100  # Was 200

    # Exclude problematic stocks that caused 2024-2025 losses
    EXCLUDE_TICKERS = {"ILMN", "SBUX", "NKE", "TGT", "LULU"}
    ...
    def initialize(self):
        ...
        self.set_warmup(timedelta(days=150))  # Shorter warmup for 100 SMA
        ...
        if len(candidates) < self.TOP_N:
            for symbol in self.stocks:
                ...
                if self.is_in_cooldown(symbol):
                    continue
                mom, rsq = self.calculate_momentum(symbol)
                if mom is not None and mom > 25 and rsq is not None and rsq > 0.40:
                    rel_str = self.calculate_relative_strength(symbol)
