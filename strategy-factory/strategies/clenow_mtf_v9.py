"""
Clenow Momentum v9: Boosted Returns with Controlled DD

v8 achieved 23.8% DD (great!) but only 17.6% CAGR
v9 adjustments:
1. Add 1.3x leverage (modest boost)
2. Relax sector limits (3 per sector)
3. Less aggressive volatility scaling
4. Keep ATR trailing stop (helps DD)
"""
...
class ClenowMTFv9(QCAlgorithm):
    """
    v9: Boosted returns with DD control
    ...
    # v9 CHANGES
    LEVERAGE = 1.3  # Modest leverage (was 1.0)
    MAX_PER_SECTOR = 3  # Relaxed from 2
    VOL_SCALE_FACTOR = 0.5  # Less aggressive (0.5 = half impact)
    ...
    MAX_POSITION_SIZE = 0.50  # Allow bigger positions
    ...
    def get_volatility_weight(self, symbol):
        ...
        # Less aggressive: Use VOL_SCALE_FACTOR to reduce impact
        # If factor = 0.5, weight ranges from 0.75 to 1.5 instead of 0.5 to 2.0
