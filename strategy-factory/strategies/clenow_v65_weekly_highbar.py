"""
Clenow v65: Weekly Rebalance + Higher Bar

Based on v39 (23.6% CAGR, 23.2% DD)
Changes:
- Weekly rebalancing (instead of bi-weekly)
- Higher momentum threshold: 60 (from 45)
- Higher rel strength: 20 (from 12)
- More selective = higher quality positions

Hypothesis: More frequent rebalancing + higher bar = catch more moves
"""
...
class ClenowWeeklyHighBar(QCAlgorithm):

    MOMENTUM_LOOKBACK = 50
    TOP_N = 3
    MIN_MOMENTUM = 60  # Higher bar
    MIN_REL_STRENGTH = 20  # Must strongly beat SPY
    MIN_R_SQUARED = 0.55  # Higher quality
        ...
        if len(candidates) < self.TOP_N:
            for symbol in self.stocks:
                if not self.is_uptrending(symbol):
                    continue
                mom, rsq = self.calculate_momentum(symbol)
                if mom is not None and mom > 30:  # Slightly higher fallback
                    rel_str = self.calculate_relative_strength(symbol)
