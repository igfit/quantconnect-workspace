"""
Clenow v34: Hyper-Beta with Looser Portfolio DD Protection

v33 failed with 12% DD limit - too aggressive for volatile stocks
v34: 18% DD limit with faster recovery re-entry

Target: Reduce DD from 37% (v29) to <30% while keeping CAGR high
"""
...
class ClenowLooseDD(QCAlgorithm):
    ...
    # Looser portfolio DD protection
    PORTFOLIO_DD_LIMIT = 0.18  # Exit if portfolio drops 18% (was 12%)
    RECOVERY_THRESHOLD = 0.03  # Re-enter faster (was 5%)
    ...
    def rebalance(self):
        ...
        self.log(f"LOOSE DD: {', '.join([f'{s.value}' for s, _ in top])}")
