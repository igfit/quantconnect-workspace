"""
Clenow v53: High Conviction Filter

Hypothesis: Only trade when momentum signals are very strong.
Higher MIN_MOMENTUM threshold but with higher position sizes.

Changes from v39:
- MIN_MOMENTUM = 60 (was 45) - only trade strong momentum
- MIN_REL_STRENGTH = 20 (was 12) - must beat SPY significantly
- Stay invested when no candidates (don't sell existing winners)
"""
...
class ClenowHighConviction(QCAlgorithm):

    MOMENTUM_LOOKBACK = 50
    TOP_N = 3
    MIN_MOMENTUM = 60  # Higher threshold
    MIN_REL_STRENGTH = 20  # Higher relative strength required
    MIN_R_SQUARED = 0.55  # Require smoother trends
    ...
    def rebalance(self):
        ...
        # KEY CHANGE: If no high-conviction candidates, KEEP existing holdings
        if len(candidates) < self.TOP_N:
            if len(self.current_holdings) > 0:
                return  # Keep existing winners
            # Fallback only if we have no positions
            for symbol in self.stocks:
                if not self.is_uptrending(symbol):
                    continue
                mom, rsq = self.calculate_momentum(symbol)
