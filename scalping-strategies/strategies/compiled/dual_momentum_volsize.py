"""
Dual Momentum Strategy - Volatility-Adjusted Position Sizing

Instead of fixed $20K per position, size based on ATR
Lower volatility = larger position, Higher volatility = smaller position
Target: Equalize risk across positions
"""

from AlgorithmImports import *


class DualMomentumVolSize(QCAlgorithm):
    ...
    def initialize(self):
        ...
        self.base_risk_per_trade = 2000  # Risk $2K per trade (2% of capital)
    ...
    def calculate_position_size(self, symbol, price, atr_value):
        """Size position based on ATR - higher volatility = smaller position"""
        if atr_value <= 0:
            return 0

        # Risk per share = 2 * ATR (using trailing stop logic)
        ...
        max_value = self.portfolio.total_portfolio_value * 0.20
    ...
    def on_end_of_algorithm(self):
        self.log(f"Dual Momentum VolSize: ${self.portfolio.total_portfolio_value:,.2f}")
